jQuery(function($){
	$.extend({
				...
				if(response.errcode != 0){
					...
					if(action_type == 'list' || action_type == 'left'){
						...
						if($('#TB_ajaxContent').length > 0){
							...
							$('#tb_modal').wpjam_show_modal(response.form, response.page_title, response.width);
						}
					}else{
		...
		wpjam_list_table_response: function(response, args){
			if(response.type == 'items' && response.items){
				...
				if($('#TB_ajaxContent').length > 0){
					...
					$('#tb_modal').wpjam_show_modal(response.data, response.page_title, response.width);
				}
			}else{
				if($('#TB_ajaxContent').length > 0){
					if(response.dismiss){
		...
		wpjam_list_table_query_items: function(type){
			return $.wpjam_list_table_action({
		...
		wpjam_list_table_loaded: function(){
			...
			wpjam_page_setting.list_table.loaded	= true;
			...
			if(wpjam_page_setting.list_table.sortable){
				let items	= wpjam_page_setting.list_table.sortable.items  || ' > tr';

				$('.wp-list-table > tbody:not(.ui-sortable)').sortable({
					items:		items,
					axis:		'y',
			...
			if($.inArray(wpjam_page_setting.screen_base, ['edit', 'upload', 'edit-tags']) != -1 && wpjam_page_setting.list_table.ajax){
				let page	= new URL($('#adminmenu a.current').prop('href'));

				$('body .subsubsub a, body tbody#the-list a').addClass(function(){
					if(!$(this).prop('href') || $(this).prop('href').indexOf('javascript:;') === 0){
						return;
					}
			...
			$.post(ajaxurl, args, function(data, status){
				...
				if(response.errcode != 0){
					...
					if(action_type == 'submit'){
						...
						$('#tb_modal').wpjam_show_modal(response_form, response.page_title, response.width);
					}else{
	...
	$.fn.extend({
		wpjam_show_modal: function(html, title, width){
			if($(this).attr('id')){
				if(html){
					$(this).html(html);
				}

				width	= width || $(this).data('width') || 720;
				title	= title || $(this).data('title') || ' ';

				tb_show(title, '#TB_inline?inlineId='+$(this).attr('id')+'&width='+width);
			}
		},

		wpjam_list_table_href: function(){
			let href	= new URL($(this).prop('href'));
			let except	= 'post_type';

			if(wpjam_page_setting.screen_base == 'edit-tags'){
				except 	= 'taxonomy';
			}
			...
			for(let [key, value] of href.searchParams.entries()){
				if(key != except){
					wpjam_page_setting.params[key]	= value;
				}
			}
		...
		wpjam_list_table_action: function(){
		...
		wpjam_list_table_bulk_action: function(){
			let bulk_action	= $(this).val();
			let bulk_option	= $(this).find('option:selected');
		...
		wpjam_list_table_column_sort: function(){
			let href = new URL($(this).prop('href'));

			let orderby	= href.searchParams.get('orderby');
			let order	= href.searchParams.get('order');

			wpjam_page_setting.params.orderby	= orderby || $(this).parent().attr('id');
			wpjam_page_setting.params.order		= order || ($(this).parent().hasClass('asc') ? 'desc' : 'asc');
			wpjam_page_setting.params.paged		= 1;

			return $.wpjam_list_table_query_items();
		},

		wpjam_list_table_left_paginate: function(){
			...
			wpjam_page_setting.params.left_paged	= paged;
	...
	$('body').on('click', '.show-modal', function(){
		if($(this).data('modal_id')){
			$('#'+$(this).data('modal_id')).wpjam_show_modal();
		}
	});

	if($('#notice_modal').length){
		$('#notice_modal').wpjam_show_modal();
	}
	...
	window.onpopstate = function(event){
		if(event.state && event.state.wpjam_params){
			...
			if(wpjam_page_setting.params.page_action){
				$.wpjam_page_action($.extend({}, wpjam_page_setting.params, {action_type: 'form'}));
			}else if(wpjam_page_setting.params.list_action && wpjam_page_setting.list_table){
				$.wpjam_list_table_action($.extend({}, wpjam_page_setting.params, {action_type: 'form'}));
			}else{
				tb_remove();
	...
	if(wpjam_page_setting.list_table && wpjam_page_setting.list_table.form_id){
		...
		$('body').on('submit', list_table_form, function(e){
			...
			if(active_element_id == 'doaction' || active_element_id == 'doaction2'){
				return $('#'+active_element_id).prev('select').wpjam_list_table_bulk_action();
			}

			if(wpjam_page_setting.list_table.ajax){
		...
		$('body').on('click', '.list-table-action', function(){
			return $(this).wpjam_list_table_action();
		});

		$('body').on('click', '.list-table-href', function(){
			return $(this).wpjam_list_table_href();
		});
		...
		if(wpjam_page_setting.list_table.ajax){
			$('body').on('click', list_table_form+' .pagination-links a', function(){
				wpjam_page_setting.params.paged	= (new URL($(this).prop('href'))).searchParams.get('paged');

				return $.wpjam_list_table_query_items();
			});

			$('body').on('click', list_table_form+' th.sorted a, '+list_table_form+' th.sortable a', function(){
				return $(this).wpjam_list_table_column_sort();
			});
		}

		$('body').on('click', '#col-left .left-pagination-links a', function(){
			return $(this).wpjam_list_table_left_paginate();
		});
