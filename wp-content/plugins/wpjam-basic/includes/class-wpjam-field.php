...
class WPJAM_Field{
	...
	public static function validate($field, $value, $validate=true){
		...
		$type		= $field['type'];
		$title		= !empty($field['title']) ? '「'.$field['title'].'」' : '「'.$field['key'].'」';
		...
		if($type == 'checkbox'){
			...
			if(empty($value) && !is_numeric($value) && $required){
				...
				if($type == 'radio'){
					...
					if(!is_null($value)){
						if($validate){
							if(!empty($field['minlength']) && is_numeric($field['minlength'])){
								if(mb_strlen($value) < $field['minlength']){
									return new WP_Error('invalid_value', $title.'的长度小于最小长度'.$field['minlength']);
								}
							}

							if(!empty($field['maxlength']) && is_numeric($field['maxlength'])){
								if(mb_strlen($value) > $field['maxlength']){
									return new WP_Error('invalid_value', $title.'的长度大于最大长度'.$field['maxlength']);
								}
							}
						}
		...
		if(!empty($field['data_type']) && $value){
			$value	= apply_filters('wpjam_data_type_field_value', $value, $field);
		}
		...
		if($type == 'view' || $type == 'br'){
			...
			if(current_user_can('upload_files')){
				...
				foreach($value as $img){
					...
					$img_tag	= '<img width="100" src="'.wpjam_get_thumbnail($img_url, 200, 200).'" alt="'.($field['title'] ?? ' ').'">';
	...
	private static function render_input($field){
		...
		$keys	= ['type','key','title','value','default','description','options','fields','size','show_admin_column','sortable_column','data_type','item_type','total','max_items','sep','wrap_class','show_if','show_if_key','creatable','post_type','taxonomies','taxonomy','settings','sanitize_callback','validate_callback','column_callback','value_callback'];
