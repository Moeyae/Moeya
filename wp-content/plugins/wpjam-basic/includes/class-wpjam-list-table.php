...
class WPJAM_List_Table extends WP_List_Table{
	...
	public function parse_args($args){
		...
		if(!empty($args['bulk_actions'])){
			if(!wp_doing_ajax()){
				...
				if($_POST['action'] == 'wpjam-list-table-action' && $_POST['action_type'] == 'list'){
	...
	public function get_row_action($action, $args=[]){
		...
		if(!empty($action['redirect'])){
			...
			$data_attr	= $data ? 'data-filter=\''.wpjam_json_encode($data).'\'' : '';
		}else{
			$class		= in_array($action['response'],['move', 'move_item']) ? 'list-table-move-action' : 'list-table-action';
	...
	public function ajax_response(){
		$referer_parts	= parse_url(wpjam_get_referer());
