...
class WPJAM_Thumbnail{
	...
	public function get_default($size='full', $crop=1){
		if($thumbnail_url = apply_filters('wpjam_default_thumbnail_url', $this->get_setting('default'))){
	...
	public function get_term_field(){
		if($taxonomies = $this->get_setting('term_thumbnail_taxonomies')){
	...
	public function get_by_orders($post){
		$orders	= $this->get_setting('post_thumbnail_orders', []);
	...
	public static function filter_post_thumbnail_url($thumbnail_url, $post){
		if($_thumbnail_url = self::get_instance()->get_by_orders($post)){
...
function wpjam_get_default_thumbnail_url($size='full', $crop=1){	// 默认缩略图
	return WPJAM_Thumbnail::get_instance()->get_default($size, $crop);
}

add_action('wp_loaded', function(){
	$instance	= WPJAM_Thumbnail::get_instance();
	
	if($field = $instance->get_term_field()){
