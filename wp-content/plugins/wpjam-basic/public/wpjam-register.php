...
function wpjam_register_term_option($key, $args=[]){
	...
	return WPJAM_Term_Option::register($key, apply_filters('wpjam_register_term_option_args', $args, $key));
}
...
function wpjam_register_bind($name, $appid, $args){
	return WPJAM_Bind_Type::register($name, $appid, $args);
}
...
function wpjam_register_path($page_key, ...$args){
	if(count($args) == 2){
		return WPJAM_Path::register($page_key, $args[0], $args[1]);
	}else{
		$args	= $args[0];
...
function wpjam_register_theme_upgrader($upgrader_url){
	$object	= WPJAM_Upgrader::register(get_template(), ['upgrader_url'=>$upgrader_url]);
