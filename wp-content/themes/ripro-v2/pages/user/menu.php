...
<div class="card mb-lg-4 mb-2">
  <div class="card-body">
    ...
    <?php if ( _cao('is_site_qiandao',true) ) : 
      ...
      <button class="btn btn-sm btn-link w-100 mt-3 go-user-qiandao" <?php echo $disabled;?> data-nonce="<?php echo wp_create_nonce('rizhuti-v2-click-' . $current_user->ID); ?>" data-toggle="tooltip" data-placement="bottom" title="<?php echo sprintf(__('每日签到奖励: %s', 'ripro-v2'), _cao('site_qiandao_coin_num','0.5').site_mycoin('name') );?>"><i class="fa fa-check-square-o"></i> <?php echo $btntext;?></button>
    <?php endif; ?>
...
jQuery(function() {
    ...
    $(".go-user-qiandao").on("click", function(event) {
        ...
        rizhuti_v2_ajax({
            ...
            if (data.status == 1) {
                rizhuti_v2_toast_msg('success', data.msg, function() {location.reload()})
            } else {
                rizhuti_v2_toast_msg('info',data.msg,function(){location.reload()})
            }
        },function(complete) {
            _this.html(deft)
        });
