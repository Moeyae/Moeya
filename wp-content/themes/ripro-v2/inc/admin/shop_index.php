...
function query_day($star,$end){
	global $wpdb;
	$sql = $wpdb->get_var("SELECT SUM(order_price) FROM {$wpdb->cao_order} WHERE create_time > {$star} AND create_time < {$end}");
	$sql = ($sql) ? sprintf("%.2f",$sql) : 0 ;
	$sql_ok = $wpdb->get_var("SELECT SUM(order_price) FROM {$wpdb->cao_order} WHERE create_time > {$star} AND create_time < {$end} AND status=1");
	...
	<div class="layui-row layui-col-space15">
			...
			<div class="layui-card">
		        ...
		        <div class="layui-card-body">
					...
					<?php foreach ($arr_itme as $key => $item) { 
						...
						$_sum = $wpdb->get_var("SELECT SUM(order_price) FROM {$wpdb->cao_order} WHERE create_time > {$_time[0]} AND create_time < {$_time[1]}");
						$_sum = ($_sum) ? $_sum : 0 ;
						$_sum_ok = $wpdb->get_var("SELECT SUM(order_price) FROM {$wpdb->cao_order} WHERE create_time > {$_time[0]} AND create_time < {$_time[1]} AND status=1");
			...
			<div class="layui-card">
		        ...
		        <div class="layui-card-body">
		         <ul class="layui-row layui-col-space10 layui-this">
		         	...
		         	$value_e = (float)$wpdb->get_var($wpdb->prepare("SELECT SUM(meta_value) FROM $wpdb->usermeta WHERE meta_key=%s", 'cao_balance'));

		         	$value_f = (float)$wpdb->get_var($wpdb->prepare("SELECT SUM(meta_value) FROM $wpdb->usermeta WHERE meta_key=%s", 'cao_total_bonus'));
		         	...
		         	$info_arr = [
		         		...
		         		array('title' => '累计佣金池','value' => $value_f ),
		...
		<div class="layui-col-md12">
			...
			<div class="layui-row layui-col-space15">
				...
				<div class="layui-col-md3">
					<div class="layui-card">
						<div class="layui-card-header">资源销量排行(前十)</div>
						<div class="layui-card-body">
							<dl class="layuiadmin-card-status">
								<?php $result = $wpdb->get_results("select post_id,count(post_id) as sum_pay_num,sum(order_price) as sum_order_amount from {$wpdb->cao_order} where order_type='other' group by post_id,status having status=1 and sum(order_price) >0 order by count(post_id) desc limit 10");
								foreach ($result as $key => $value) {
									echo '<dd>';
									...
									echo '<span>单价：￥'.get_post_meta($value->post_id,'cao_price', 1).' | 总销售额：￥'.$value->sum_order_amount.'</span>';
									echo '</div>';
									echo '</dd>';
								}?>
				            </dl>
				...
				<div class="layui-col-md3">
					<div class="layui-card">
						...
						<div class="layui-card-body">
							<dl class="layuiadmin-card-status">
								<?php $result = $wpdb->get_results("select user_id,meta_value from $wpdb->usermeta where meta_key='cao_balance' group by user_id having meta_value>0 order by meta_value desc limit 10");
								foreach ($result as $key => $value) {
									echo '<dd>';
