	...
	<div id="post-body-content">
        <?php if ($page_action=='addcdk'): ?>
            ...
            if (isset($_POST['add_money']) && $_POST['add_day'] && $_POST['add_num']) {
                ...
                echo '<div class="updated notice notice-success is-dismissible"><p>' . sprintf( __('成功添加: %d 条卡密', 'ripro-v2'), $_POST['add_num'] ) . '</p></div>';
            }?>
            <form action="" id="poststuff" method="post" name="post_add_cdk">
                ...
                <table class="form-table">
                    <tbody>
                        ...
                        <tr>
                            <th scope="row"><label for="add_money">卡密金额(站内币)</label></th>
                            <td><input id="add_money" min="1" name="add_money" step="1" type="number" value="1"> <?php echo site_mycoin('name')?></input></td>
                        </tr>
                    </tbody>
                </table>
        ...
        <?php elseif ('outcdk' === $RiPlusTable->current_action()): ?>
            ...
            foreach ($outcdk_ids as $key => $cdkid) {
                $cdk = $wpdb->get_var($wpdb->prepare("SELECT code FROM {$wpdb->cao_coupon} WHERE id = %d ", $cdkid));
...
class RiPlus_List_Table extends WP_List_Table
{
    ...
    public function get_columns()
    {
       $columns = [
			...
			'code'    => __( '卡密代码', 'ripro-v2' ),
            'code_type'    => __( '卡密类型', 'ripro-v2' ),
			'sale_money'    => __( '卡密面值(站内币)', 'ripro-v2' ),
			'create_time'    => __( '创建时间', 'ripro-v2' ),
			'end_time'    => __( '到期时间', 'ripro-v2' ),
            'apply_time'    => __( '使用时间', 'ripro-v2' ),
    ...
    public function column_default( $item, $column_name )
    {
        switch ( $column_name ) {
            ...
            case 'sale_money':
            	if (true) {
                    return '<span class="badge badge-hollow">'.$item[$column_name].site_mycoin('name').' | ￥'.convert_site_mycoin($item[$column_name],'rmb').'</span>';
                }else{
    ...
    private function table_data($per_page = 5, $page_number = 1 )
    {
		...
		$sql = "SELECT * FROM {$wpdb->cao_coupon} WHERE 1=1";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
    ...
    private function table_data_count() {
        ...
        $sql = "SELECT COUNT(*) FROM {$wpdb->cao_coupon} WHERE 1=1";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$sql .= ' AND code="' . trim($_REQUEST['s']) . '"';
		}
