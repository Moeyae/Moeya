...
if ($page_action=='delete_log' && $wpdb->delete( "$wpdb->cao_order", array('order_type' => 'charge','status' => 0))) {
...
class RiPlus_List_Table extends WP_List_Table{
    ...
    public function column_default( $item, $column_name ){
        switch ( $column_name ) {
            ...
            case 'order_type':
            	return get_order_type_text((object)$item);
            case 'pay_num':
            	if (!empty($item[$column_name])) {
    ...
    private function table_data($per_page = 5, $page_number = 1 ){
		...
		$sql = "SELECT * FROM {$wpdb->cao_order} WHERE order_type='charge'";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$user_id = 0;
    ...
    private function table_data_count() {
        ...
        $sql = "SELECT COUNT(*) FROM {$wpdb->cao_order} WHERE order_type='charge'";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$user_id = 0;
			if (is_numeric($_REQUEST['s'])) {
