...
class RiPlus_List_Table extends WP_List_Table
{
    ...
    public function get_columns()
    {
       $columns = [
			...
			'user_type'    => __( '用户等级', 'ripro-v2' ),
			'down_id'    => __( '下载名称', 'ripro-v2' ),
			'ip'    => __( '下载IP', 'ripro-v2' ),
            // 'note'    => __( '备注', 'ripro-v2' ),
            'create_time'    => __( '下载时间', 'ripro-v2' ),
            'today_count_num'    => __( '今日可下载', 'ripro-v2' ),
            'today_down_num'    => __( '已下载次数', 'ripro-v2' ),
            'over_down_num'    => __( '剩余次数', 'ripro-v2' ),
		];
    ...
    public function column_default( $item, $column_name )
    {
        switch ( $column_name ) {
            ...
            case 'down_id':
            	if (get_permalink($item[$column_name])) {
            		return '<a target="_blank" href='.get_permalink($item[$column_name]).'>'.get_the_title($item[$column_name]).'</a>'; 
            	}else{
            		return '<span style="color:red;">【文章ID-'.$item[$column_name].'】-已删除</span>'; 
            	}
    ...
    private function table_data($per_page = 5, $page_number = 1 )
    {
		...
		$sql = "SELECT * FROM {$wpdb->cao_down_log} WHERE 1=1";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$user_id = 0;
    ...
    private function table_data_count() {
        ...
        $sql = "SELECT COUNT(*) FROM {$wpdb->cao_down_log} WHERE 1=1";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$user_id = 0;
			if (is_numeric($_REQUEST['s'])) {
