...
function toggle_dark() {
    ...
    if ($is_open_dark == 1 && empty(RiSession::get('site_dark_open'))) {
        $status = 1;
    } else {
        $status = 0;
    }

    RiSession::set('site_dark_open', $status);
...
function seav_userinfo() {
    ...
    $phone       = !empty($_POST['phone']) ? esc_sql($_POST['phone']) : null;
...
function go_add_reflog() {
    ...
    $money = !empty($_POST['type']) ? (int)$_POST['money'] : 0;
...
function go_post_pay() {
    ...
    $order_data = [
        ...
        'order_name'     => get_bloginfo('name') . esc_html__('-自助购买', 'ripro-v2'),
...
function go_coin_pay() {
    ...
    $order_data = [
        ...
        'pay_type'       => $pay_type, //支付方式
        'order_name'     => get_bloginfo('name') . esc_html__('-自助充值', 'ripro-v2'),
        'callback_url'   => get_user_page_url(),
        'order_info'     => maybe_serialize($order_info),
    ];
...
function check_mpweixin_qr() {
    ...
    // 查询数据库
    $res = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->cao_mpwx_log WHERE scene_id = %s AND openid != '' ", esc_sql($scene_id)));
...
function get_async_shop_down() {
    ...
    if ($IS_PAID == 0) {
        ...
        if ($the_user_type == 'nov' && $the_post_price == -1) {
            ...
            echo '<button type="button" class="btn btn-block btn-primary mb-3 login-btn">' . esc_html__('登录后购买', 'ripro-v2') . '</button>';
        } else {
            echo '<button type="button" class="btn btn-block btn-primary mb-3 click-pay-post" data-postid="' . $post_id . '" data-nonce="' . $click_nonce . '" data-price="' . $the_post_price . '">' . esc_html__('立即购买', 'ripro-v2') . '</button>';
        }
