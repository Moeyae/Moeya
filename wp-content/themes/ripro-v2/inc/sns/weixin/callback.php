...
if (!RiSession::get('RIPLUS_WEIXIN_STATE', 0)) {
    ripro_wp_die('非法访问，没有经过第三方登录返回');
}

$is_get_openid = RiSession::get('is_get_weixin_openid', 0);
...
try {
    //获取AccessToken
    $AccessToken = $wxOAuth->getAccessToken(RiSession::get('RIPLUS_WEIXIN_STATE', 0));
} catch (Exception $e) {
...
if ($is_get_openid) {
    RiSession::set('current_weixin_openid', $openid);
    wp_safe_redirect(urldecode(RiSession::get('sns_rurl')));exit;
}
...
$RiProSNS->go_callback($wexin_key_type, $openid, $snsInfo, $mpwx_get_user_id_type);
