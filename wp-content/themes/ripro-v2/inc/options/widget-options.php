    ...
    function rizhuti_v2_module_dynamic($args, $instance) {
        ...
        foreach ($data as $key => $value) {
            ...
            if (mb_strlen($u_name) > 2) {
                $u_name = mb_substr_replace($u_name, '*', 1, -1);
            } else {
                $u_name = mb_substr_replace($u_name, '*', 0, -1);
            }
        ...
        <div class="row">
            ...
            <?php 
                foreach ($instance['btn_cat'] as $_cid) {
                    ...
                    if (!$item) { continue; }
                    echo '<a target="_blank" class="btn btn-sm btn-light mr-2 mb-lg-4 mb-2" href="' . get_category_link($_cid) . '">' . $item->name . '</a>';
                }
            ?>
            </div>
        </div>
    ...
    function rizhuti_v2_module_post_item($args, $instance) {
        ...
        <div class="row">
            ...
            if (!empty($instance['is_child_cat_ajax']) && !empty($instance['category'])) {
                ...
                if (!empty($child_categories)) {
                  echo '<a target="_blank" class="btn btn-sm btn-light mr-2 mb-lg-4 mb-2 active" href="' . get_category_link($instance['category']) . '">' . esc_html__('全部','ripro-v2') . '</a>';
                  foreach ($child_categories as $item) {
                      echo '<a target="_blank" class="btn btn-sm mb-2 btn-light mr-2 mb-lg-4 mb-2" href="' . get_category_link($item->term_id) . '">' . $item->name . '</a>';
                  }
                }
            }
            ?>
        ...
        <div class="module posts-wrapper <?php echo esc_attr($instance['item_style']); ?>">
            ...
            <div class="w-100 text-center">
                <a href="<?php echo get_category_link($instance['category']);?>" class="btn btn-light"><i class="fas fa-angle-double-right"></i> <?php echo esc_html__('查看更多','ripro-v2');?></a>
            </div>
