<?php
/*
 * @Description: Modifier functions in docs list table
 * @Version: 2.6.9
 * @Author: ZAXU
...
class zaxuDocs_Docs_List_Table {
    ...
    function add_meta_box() {
        add_meta_box('op-menu-meta-box-id', __('Helpfulness', 'zaxu'), array($this, 'helpfulness_metabox'), 'docs', 'side', 'core');
    }
    ...
    function helpfulness_metabox() {
        ...
        <table style="width: 100%;">
            <tr>
                <td class="zaxudocs-positive">
                    <span class="dashicons dashicons-thumbs-up"></span>
                    <?php printf( '%d', get_post_meta($post->ID, 'positive', true) ); ?>
                </td>

                <td class="zaxudocs-negative" style="text-align: right;">
    ...
    public function docs_list_columns($columns) {
        $vote  = array( 'votes' => __('Helpfulness', 'zaxu') );
    ...
    public function docs_list_columns_row($column_name, $post_id) {
        if ('votes' == $column_name) {
            ...
            printf( '<span class="zaxudocs-positive">%d</span>/<span class="zaxudocs-negative">%d</span>', $positive, $negative );
        }
    }
