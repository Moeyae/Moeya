<?php
/*
 * @Description: All single portfolio type
 * @Version: 2.7.1
 * @Author: ZAXU
...
<?php zaxu_wrapper_start(); ?>
	<?php
		if ( have_posts() ) {
			...
			if (get_theme_mod('zaxu_portfolio_details_page_style', 'journal') == "journal") {
				// Journal details page start
					...
					if ( !post_password_required() ) {
						// Get post tag start
							if (get_theme_mod('zaxu_portfolio_tag', "enabled") == "enabled") {
						...
						// Get rating start
							if ( !is_attachment() ) {
								if ( get_theme_mod('zaxu_portfolio_rating') != 'disabled') {
					...
					if (get_theme_mod('zaxu_portfolio_details_page_attr_info', 'enabled') == "enabled") {
						echo '
					...
					echo '
							...
							<footer class="entry-footer">' . zaxu_post_navigation("portfolio") . '</footer>
						</article>
					';
