...
define('B2_HOME_URI',get_option('home'));
...
//获取主题版本号
// function b2_get_theme_version(){
//     //$theme = wp_get_theme();
//     return '1.0';
// }

//add_action( 'load-themes.php', 'b2_install' );

function b2_force_permalink(){
...
add_action('load-themes.php', 'b2_force_permalink');
...
if ( ! function_exists( 'b2_setup' ) ) :
    ...
    function b2_setup() {
		...
		$arg['post'] = __( '首页、文章、页面、分类、专题等页面菜单', 'b2' );
		
		if(b2_get_option('newsflashes_main','newsflashes_open')){
			$arg['newsflashes'] = __( '快讯页面菜单', 'b2' );
		}
		...
		if(b2_get_option('shop_main','shop_open')){
			$arg['shop'] = __( '商铺页面菜单', 'b2' );
		}

		if(b2_get_option('circle_main','circle_open')){
			$arg['circle'] = __( '圈子页面菜单', 'b2' );
		}
...
add_action( 'init', 'b2_disable_emojis' );
...
function b2_get_option($where,$key,$circle_id = 0){
    return apply_filters('b2_get_option',B2\Modules\Settings\Main::get_option($where,$key,$circle_id),$where,$key,$circle_id);
}
...
function b2_get_default_img(){
	...
	if(empty($default_img)){
		$default_img = array(
			B2_THEME_URI.'/Assets/fontend/images/default-img.jpg'
		);
	}
...
function b2_timeago($ptime){
	return B2\Modules\Common\Post::time_ago($ptime);
}
...
function b2_get_search_type(){
	$arg = apply_filters('b2_custom_post_type', array(
		...
		'post'=>__('文章', 'b2'),
		'user'=>__('用户', 'b2'),
		'shop'=>b2_get_option('shop_main','shop_name'),
		'document'=>b2_get_option('document_main','document_name'),
		'newsflashes'=>b2_get_option('newsflashes_main','newsflashes_name'),
		'circle'=>__('圈子话题','b2')
		// 'bubble'=>__('冒泡', 'b2'),
		// 'labs'=>__('研究所', 'b2'),
	));
...
function b2_get_inv_settings(){
	...
	return array(
		'type'=>$invitation,
		'text'=>$invitation_text[0],
		'link'=>$invitation_text[1]
	);
}
...
function b2_order_type(){
	return apply_filters('b2_order_type',array(
		...
		'v'=>__('视频购买','b2'),
		'verify'=>__('认证付费','b2'),
		'circle_join'=>__('付费入圈','b2'),
		'circle_read_answer_pay'=>__('付费查看圈子问答','b2'),
...
function b2_text($data = array()){
	...
	return array(
		'circle'=>array(
			...
			'create_circle_role_money_error'=>__('请至少设置一个金额','b2'),
			'create_circle_role_join_error'=>__('请选择入圈规则','b2'),
			'create_circle_info_error'=>__('请完善圈子资料','b2'),
			'create_circle_info_name_error'=>__('圈子名称必须大于2小于20个字符','b2'),
			'create_circle_info_desc_error'=>__('圈子描述必须大于20小于100个字符','b2'),
			'create_circle_role_lv_error'=>__('请选择允许入圈的用户组','b2'),
			...
			'join_success'=>__('恭喜，您已成功加入本圈','b2'),
			'join_title'=>__('加入圈子','b2'),
			'circle_date'=>array(
			...
			'join_comment'=>__('加入圈子后方可查看评论','b2'),
			'delete_answer'=>__('确定要删除这个回答吗？'),
			'delete_comment'=>__('确定要删除这条评论吗？')
		),
		'global'=>array(
			'not_allow'=>__('权限不足','b2'),
			'dark_room_down'=>__('小黑屋反思中，不准下载！','b2'),
			'comment_down'=>__('评论并刷新后下载！','b2'),
			'role_down'=>__('没有权限下载，请升级会员！','b2'),
			...
			'all'=>__('全部','b2'),
			'newsflashe_r'=>__('快讯标题和内容必填','b2'),
			'newsflashe_insert_success'=>__('快讯发布成功','b2'),
			...
			'tuya'=>__('涂鸦','b2'),
			'tx_alert'=>__('申请之后余额会扣除提现部分，稍后由我们人工为您提现，确认要提现吗？','b2')
		),
		'b2_text'=>array(
			...
			'order_credit_empty'=>__('您的积分余额不足','b2'),
			...
			'check_dark_room_why'=>__('恶意频繁请求本站接口','b2'),
...
function b2_roles_arg(){
	...
	$arg = array(
		...
		'post'=>__( '发布文章', 'b2' ),
		'comment'=>__( '发布评论', 'b2' ),
		'newsflashes'=>__( '发布快讯', 'b2' ),
	);

	if(b2_get_option('circle_main','circle_open')){
		$arg['circle_topic'] = __( '圈子发帖', 'b2' );
		$arg['circle_create'] = __( '创建圈子', 'b2' );
	}
...
function b2_get_page_width($show_widget){
	...
	if($show_widget){
		...
		return $page_width - B2_SIDEBAR_GAP - $width;
...
function b2_oauth_types($no_mp = false){
	$wx_pc = 'https://open.weixin.qq.com/connect/qrconnect?appid='. b2_get_option('normal_login','wx_pc_key') .'&response_type=code&scope=snsapi_login&redirect_uri='. urlencode (home_url('/open?type=wx_pc'));
	$wx_gz = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='. b2_get_option('normal_login','wx_gz_key') .'&&redirect_uri='. urlencode (home_url('/open?type=wx_gz')).'&response_type=code&state='.md5 ( uniqid ( rand (), true ) ).'&scope=snsapi_userinfo#wechat_redirect';
	...
	return apply_filters('b2_oauth_types_arg', array(
		'weixin'=>$wx,
		'weibo'=>array(
			'name'=>__('微博','b2'),
			'url'=>'https://api.weibo.com/oauth2/authorize?client_id=' .b2_get_option('normal_login','weibo_id'). '&response_type=code&redirect_uri=' . urlencode (home_url('/open?type=weibo')),
			'open'=>b2_get_option('normal_login','weibo_open') ? true : false
		),
		'qq'=>array(
			'name'=>__('QQ','b2'),
			'url'=>"https://graph.qq.com/oauth2.0/authorize?client_id=" .b2_get_option('normal_login','qq_id'). "&state=".md5 ( uniqid ( rand (), true ) )."&response_type=code&redirect_uri=" . urlencode (home_url('/open?type=qq')),
...
//获取用户数据
function b2_get_userdata($user_id,$type){
...
function b2_get_yun_video_poster($link){
	if(defined('OSS_ACCESS_ID')) return $link.'?x-oss-process=video/snapshot,t_1,m_fast';
...
add_filter('auth_cookie_expiration', 'b2_cookie_expiration', 99, 3);
...
// 禁用其他图片尺寸
function b2_disable_other_image_sizes() {
    
    remove_image_size('post-thumbnail'); // disable images added via set_post_thumbnail_size() 
    remove_image_size('another-size');   // disable any other added image sizes
    
}
add_action('init', 'b2_disable_other_image_sizes');
