...
<div id="comments" class="comments-area box b2-radius">
		...
		<ol class="comment-list" ref="commentList">
			<?php
				...
				if($comment_count){
					...
					echo '<div class="none-comment" ref="noneComment">'.__('暂无讨论，说说你的看法吧').'</div>';
				}else{
					echo '<div class="none-comment">'.__('关闭讨论').'</div>';
				}
			?>
		</ol><!-- .comment-list -->
		<?php 
	...
	<div class="b2-pagenav comment-nav b2-radius <?php echo get_comment_pages_count() <= 1 ? 'b2-hidden-always' : '' ?>">
		<page-nav ref="commentPageNav" paged="<?php echo $paged; ?>" navtype="comment" pages="<?php echo get_comment_pages_count($comments); ?>" type="<?php echo b2_get_option('template_comment','nav_type'); ?>" :box="selecter" :opt="opt" :api="api" url="<?php echo get_permalink(); ?>" title="<?php echo get_the_title(); ?>"></page-nav>
	</div>
</div><!-- #comments -->
