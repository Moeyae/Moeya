	...
	<footer id="colophon" class="footer">
		...
		<div class="site-footer-nav">
			<div class="wrapper">
				...
				?>
				<?php if((is_home() || is_front_page()) && !empty($link_cats)){ ?>
				...
				<div class="footer-bottom">
					<div class="footer-bottom-left">
						<div class="copyright"><?php echo 'Copyright &copy; '.date('Y').'<a href="'.B2_HOME_URI.'" rel="home">&nbsp;'.get_bloginfo('name').'</a>'; ?></div>
						<div class="beian">
							<?php if($beian){
								echo '<span class="b2-dot">・</span><a rel="nofollow" target="__blank" href="https://beian.miit.gov.cn">'.$beian.'</a>';
							} ?>
							<?php if($gongan){
								echo '<span class="b2-dot">・</span><a rel="nofollow" target="__blank" href="http://www.beian.gov.cn/portal/registerSystemInfo?recordcode='.$gongan_code.'">'.b2_get_option('template_footer','footer_gongan').'</a>';
							}?>
						</div>
					</div>
					<div class="footer-bottom-right">
						<?php
							...
							echo sprintf(__('耗时 %s 秒'),timer_stop(0,4));
						?>
					</div>
				</div>
					...
					<?php if($allow_newsflashes){ ?>
						<div>
							<button @click.stop="go('<?php echo get_post_type_archive_link('newsflashes'); ?>?action=showbox','newsflashes')">
								...
								<span class="po-post-title"><?php echo __('发布快讯','b2'); ?></span>
							</button>
						</div>
					<?php } ?>
					<?php if($allow_circle){ ?>
						<div>
							<button @click.stop="go('<?php echo b2_get_custom_page_url('create-circle'); ?>','create_circle')">
								...
								<span class="po-post-title"><?php echo __('创建圈子','b2'); ?></span>
							</button>
						</div>
						<div>
							<button @click.stop="go('<?php echo home_url('/circle'); ?>','create_topic')">
