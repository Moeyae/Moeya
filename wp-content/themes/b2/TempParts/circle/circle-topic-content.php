        ...
        <div class="topic-name">
            <div>
                <div class="topic-name-data">
                    ...
                    <span v-if="item.author.is_circle_admin" class="circle-is-admin"><?php echo __('圈主','b2'); ?></span>
...
<div class="topic-content">
    <h2 v-if="item.title || item.best" v-cloak>
        <a :href="item.link" target="_blank" ><span v-html="item.title" v-if="item.title"></a></span>
    ...
    <template v-if="!item.allow_read['allow']">
        ...
        <div :class="'topic-read-role '+item.allow_read['type']" v-if="item.allow_read['type'] === 'current_circle_read'">
            <div class="topic-guess-box allow-read-type login b2-radius">
                <div class="topic-vote-desc"><?php echo b2_get_icon('b2-lock-2-line').__('隐藏内容，入圈后阅读','b2'); ?></div>
                <div class="not-allow-role">
                    <div class="topic-read-nub">
                        <div class="topic-read-number"><?php echo __('您需要入圈之后才能查看帖子内容','b2'); ?></div>
                    </div>
                    <div class="topic-read-pay-button">
                        <button class="empty" @click="jionCircleAction(item)"><?php echo __('加入圈子','b2'); ?></button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <template v-else>
        <div v-if="item.data.type === 'guess' && item.data.data.answer === item.data.data.picked" class="guess-right-tips">
            <p><?php echo sprintf(__('%s您猜对了答案，下面是向您展示的隐藏信息：%s'),'<span class="b2-color">','</span>'); ?></p>
        </div>
        ...
        <div v-if="item.attachment.video.length > 0" v-cloak class="topic-video-box" v-cloak>
            <ul>
                <li v-for="(v,index) in item.attachment.video" :key="v.id" :style="'max-width:'+v.width+'px'">
                    <div :style="'padding-top:'+(v.ratio*100)+'%'" class="b2-radius">
                        ...
                        <div class="topic-video-play" @click="play(item.topic_id,index)">
                    ...
                    <li v-for="(big,index) in item.attachment.image" :key="big.id" :class="item.attachment.imageIndex === index ? 'image-show' : 'b2-hidden-always'">
                        <div :style="'padding-top:'+(big.big_ratio*100)+'%'" class="box-in b2-radius">
                            ...
                            <?php echo b2_get_img(array(
                                ...
                                'src_data'=>':src="big.big_thumb" @click="closeImageBox(ti)"'
                            ));?>
