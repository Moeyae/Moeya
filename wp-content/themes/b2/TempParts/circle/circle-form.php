                ...
                <div class="circle-info-in">
                    <div class="circle-info-left">
                        <?php echo b2_get_img(array(
                            'source_data'=>':srcset="getCircleData(circle.picked,\'icon_webp\') ? getCircleData(circle.picked,\'icon_webp\') : \''.B2_THEME_URI.'/Assets/fontend/images/default-img.jpg\'"',
                            'src_data'=>':src="getCircleData(circle.picked,\'icon\') ? getCircleData(circle.picked,\'icon\') : \''.B2_THEME_URI.'/Assets/fontend/images/default-img.jpg\'"',
                    ...
                    <div class="circle-admin-info" v-show="showAdminBox" v-cloak>
                        ...
                        <div>
                            ...
                            <span class="b2-color"><?php echo __('所有圈子','b2'); ?></span>
                ...
                <div class="po-form-box-in" ref="poFormBox">
                    ...
                    <div class="po-topic-box-tips b2-radius" v-if="!login || !currentUser.allowCreateTopic || !currentUser.inCircle || currentUser.darkRoom || !currentUser.canPost" v-show="showPoBox" v-cloak>
                        ...
                        <div v-else-if="!currentUser.inCircle" class="join-circle-pay-money">
                            <h2>
                                <?php echo __('您还未加入该圈','b2'); ?>
                            </h2>
                            <div>
                                <div v-if="currentUser.currentCircleRole.type === 'free'">
                                    <p v-if="currentUser.currentCircleRole.data === 'check'"><?php echo __('提交申请，管理员审核之后方可入圈','b2'); ?></p>
                                    <p v-else><?php echo __('免费入圈，加入圈子后可阅读更多圈内话题','b2'); ?></p>
                                </div>
                                <div class="po-topic-box-tips-button">
                                    <button v-if="currentUser.currentCircleRole.data === 'check'" :class="currentUser.currentCircleRole.status === 'pending' ? 'pending' : ''" :disabled="currentUser.currentCircleRole.status === 'pending'" @click="joinCircle()">
                                        <span v-if="currentUser.currentCircleRole.status === 'none'"><?php echo __('提交入圈申请','b2'); ?></span>
                                        <span v-else><?php echo __('您已成功提交申请，等待审核中...','b2'); ?></span>
                                    </button>
                                    <button v-else-if="currentUser.currentCircleRole.data === 'free'" @click="joinCircle()" :class="joinLocked ? 'b2-loading' : ''" :disabled="joinLocked"><?php echo __('加入圈子','b2'); ?></button>
                                    <div v-else-if="currentUser.currentCircleRole.type === 'lv'">
                                        <p><?php echo __('专属圈子，您需要成为以下会员组的成员，方可入圈','b2'); ?></p>
                                        ...
                                        <div class="join-circle-button-pay">
                                            <button @click="joinCircle()" :disabled="!currentUser.currentCircleRole.data.allow_join">
                                                <span v-if="currentUser.currentCircleRole.data.allow_join"><?php echo __('加入圈子','b2'); ?></span>
                                    ...
                                    <div v-else>
                                        ...
                                        <div class="join-circle-button-pay">
                                            <button @click="joinPay()"><?php echo __('支付加入圈子','b2'); ?></button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="po-topic-top-left">
                            <span class="po-topic-avatar">
                                <?php echo b2_get_img(array(
                                    ...
                                    'src_data'=>':src="userData.avatar" v-if="userData.avatar"',
                                ));?>
                            </span>
                            <div class="po-ask">
                    ...
                    <div class="po-topic-textarea b2-radius" ref="textareaTopic" data-circle="<?php echo $circle_id; ?>" data-gc="<?php echo $gc; ?>">
                        ...
                        <div class="po-topic-circle" v-show="circle.show" ref="circleList" v-cloak>
                            <p class="po-topic-circle-desc"><?php echo __('可以选择广场、您加入的圈子或者您创建的圈子','b2'); ?></p>
                            <ul v-if="circle.list !== ''">
                                <li v-for="item in circle.list" :class="item.id === circle.picked ? 'picked' : ''" :key="item.id">
                                    <div class="circle-list-in" @click="circlePicked(item.id)">
                                        <div class="circle-list-image">
                                            <span>
                                                <?php echo b2_get_img(array(
                                                    'src_data'=>':src="item.icon" v-if="item.icon"',
                    ...
                    <div class="topic-po-tools">
                        <div class="circle-vote b2-radius" v-show="topicType === 'ask'" v-cloak>
                            ...
                            <div class="ask-credit">
                                <div class="vote-type">
                                    ...
                                    <button :class="ask.reward === 'money' ? 'picked' : ''" @click="ask.reward = 'money'"><?php echo __('奖励金钱','b2'); ?></button>
                                </div>
                            </div>
                            ...
                            <p class="ask-desc-text">
                                <span><?php echo sprintf(__('问题提交之后会立刻扣除相应的%s。','b2'),'<b v-if="ask.reward === \'credit\'">'.__('积分','b2').'</b><b v-else>'.__('金额','b2').'</b>'); ?></span>
                                <span>
                                    <?php echo sprintf(__('您的当前%s为：'),'<b v-if="ask.reward === \'credit\'">'.__('积分','b2').'</b><b v-else>'.__('余额','b2').'</b>'); ?>
                        ...
                        <div class="circle-media-box">
                            <div class="circle-image-box" v-if="image.list.length > 0" v-cloak>
                                <p><?php echo sprintf(__('最多上传%s张图片，可拖动排序'),'<b v-text="image.count"></b>'); ?></p>
                                <div>
                            ...
                            <div class="circle-image-box circle-video-box" v-if="video.list.length > 0" v-cloak>
                                <p><?php echo sprintf(__('最多上传%s个视频，可拖动排序'),'<b v-text="video.count"></b>'); ?></p>
                                <div>
                            ...
                            <div class="circle-file-box circle-image-box" v-if="file.list.length > 0" v-cloak>
                                <p><?php echo sprintf(__('最多上传%s个文件，可拖动排序'),'<b v-text="file.count"></b>'); ?></p>
                                <div>
                            ...
                            <div class="circle-card-box circle-image-box" v-if="card.list.length > 0" v-cloak>
                                <p><?php echo sprintf(__('最多添加%s个卡片，可拖动排序'),'<b v-text="card.count"></b>'); ?></p>
                                <div>
                            ...
                            <div :class="['modal','po-card-box',{'show-modal':card.show}]" v-cloak>
                                <div class="modal-content b2-radius">
                                    ...
                                    <div>
                                        ...
                                        <p><?php echo __('支持文章、圈子、商品、快讯、页面等网址','b2'); ?></p>
                                    </div>
                            ...
                            <div class="circle-topic-role b2-radius" v-show="role.show" v-cloak @click.stop="">
                                <div class="topic-role-type">
                                    ...
                                    <div v-if="circle.picked != circle.gc && currentUser.currentCircleRole.read !== 'private'">
                                        <label><input type="checkbox" v-model="role.currentCircle"><span><?php echo __('本圈专属','b2'); ?></span></label>
                                    </div>
                                </div>
                                <div class="topic-role-content">
                                    <div v-if="role.see === 'public'">
                                        <p><?php echo __('无限制，任意查看话题内容','b2'); ?><span v-if="role.currentCircle" class="b2-color"><?php echo __('（不在广场显示，入圈后方可查看）','b2'); ?></span></p>
                                    </div>
                                    <div v-if="role.see === 'money' && currentUser.readRole.money" class="role-content-input" v-cloak>
                                        ...
                                        <p><?php echo __('用户支付费用之后方可查看话题内容','b2'); ?><span v-if="role.currentCircle" class="b2-color"><?php echo __('（不在广场显示，入圈后方可查看）','b2'); ?></span></p>
                                    </div>
                                    <div v-if="role.see === 'credit' && currentUser.readRole.credit" class="role-content-input">
                                        ...
                                        <p><?php echo __('用户支付积分后方可查看话题内容','b2'); ?><span v-if="role.currentCircle" class="b2-color"><?php echo __('（不在广场显示，入圈后方可查看）','b2'); ?></span></p>
                                    </div>
                                    <div v-if="role.see === 'lv' && currentUser.readRole.lv" class="role-content-lv">
                                        ...
                                        <p><?php echo __('请选择允许查看本话题的等级组','b2'); ?><span v-if="role.currentCircle" class="b2-color"><?php echo __('（不在广场显示，入圈后方可查看）','b2'); ?></span></p>
                                    </div>
                                    <div v-if="role.see === 'comment' && currentUser.readRole.comment">
                                        <p><?php echo __('用户评论此话题之后方可查看话题内容','b2'); ?><span v-if="role.currentCircle" class="b2-color"><?php echo __('（不在广场显示，入圈后方可查看）','b2'); ?></span></p>
                                    </div>
                                    <div v-if="role.see === 'login' && currentUser.readRole.login">
                                        <p><?php echo __('用户登录之后方可查看话题内容','b2'); ?><span v-if="role.currentCircle" class="b2-color"><?php echo __('（不在广场显示，入圈后方可查看）','b2'); ?></span></p>
                                    </div>
                                </div>
                            </div>
                        </div>
