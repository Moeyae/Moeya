...
class Single{
    ...
    public function init(){
        ...
        //add_action('b2_single_article_after',array($this,'post_download'),4);
        ...
        //add_action('b2_single_wrapper_before',array($this,'post_download'),8);
                                ...
                                <div class="video-role-box" v-if="!user.allow">
                                    ...
                                    <div :class="\'video-role-info\'+(show ? \' b2-show\' : \'\')" v-if="user.length != 0" v-cloak>
                                        ...
                                        <div class="" v-if="user.role.type == \'money\'">
                                            ...
                                            <p>'.__('您支付费用，方可查看完整视频','b2').'</p>
                                            <div class="video-creidt">
                                                <span>'.B2_MONEY_SYMBOL.'{{user.role.value}}</span>
                                            </div>
                                            <div class="video-view-button">
                                                <button class="empty" @click="pay(\''.$can_guset_pay.'\')">'.__('支付后观看完整视频','b2').'</button>
                                                <button class="empty video-views" @click="play" v-if="showViews">'.b2_get_icon('b2-play-circle-line').__('观看预览视频','b2').'</button>
                                            </div>
                                        </div>
            ...
            <div class="post-style-3-top" style="height:'.$h.'px">
                ...
                <div class="wrapper">
                    <div class="post-style-3-title '.(!$show_sidebar ? 'content-area' : '').'">
                        <div class="">
                            '.\B2\Modules\Templates\Modules\Posts::get_post_cats('target="__blank"',$post_meta,array('cats'),'post_3').'
                        </div>
                        <h1>'.get_the_title().'</h1>
                    </div>
                </div>
    ...
    public function post_download($content){
        ...
        foreach ($download_settings as $k => $v) {
            $html .= '<div class="download-item box">
                ...
                <div class="download-list" v-cloak>
                    <div v-for="(item,index) in list" :class="\'download-item box \'+(item.current_user.can.allow ? \'allow-down\' : \'not-allow-down\')">
                        ...
                        <div class="download-rights" v-cloak>
                            <h2>'.__('下载权限','b2').'</h2><span class="mobile-show" @click="item.show_role = !item.show_role">'.__('查看','b2').'</span>
                            ...
                            <div class="download-current">
                                ...
                                <div class="" v-else>
                                    <span v-if="item.current_user.can.type == \'allow_all\'" class="green">
                                        '.__('您有每天免费下载所有资源','b2').'<b v-text="item.current_user.can.total_count"></b>'.__('次的特权，今日剩余','b2').'<b v-text="item.current_user.can.count"></b>'.__('次','b2').'
                                    </span>
                                    <span class="green" v-else>'.__('已取得下载权限','b2').'</span>
                                </div>
                            </div>
                            <div class="download-button-box">
    ...
    public function post_ds(){
        ...
        $default_settings = get_option('b2_template_single');
        $default_settings = isset($default_settings['single_ds_group']) ? $default_settings['single_ds_group'][0] : array();
        ...
        if(isset($default_settings['single_post_ds_open']) && $default_settings['single_post_ds_open'] == '1') {
            ...
            <div class="content-ds-count"><span v-if="data.count"><b v-text="data.count"></b>'.__('人已赞赏','b2').'</span><span v-else v-text="data.single_post_ds_none_text"></span></div>
    ...
    public function shop_normal_action(){
        ...
        <div class="shop-single-data-list">
            <ul>
                ...
                <li v-if="data !== '' && data[<?php echo $post_id; ?>].price.credit" v-cloak>
                    <div class="shop-single-data-title">
                        <?php echo __('赠送积分','b2'); ?>
                    </div>
                    <div class="shop-single-data-value">
        ...
        <div class="mg-t shop-single-action">
            <div class="shop-single-action-left" v-if="data !== '' && !data[<?php echo $post_id; ?>]['out_link']" v-cloak>
                <a class="button" target="_blank" :href="'<?php echo b2_get_custom_page_url('carts').'?id='.$post_id.'&count='; ?>'+count" v-if="data !== '' && data[<?php echo $post_id; ?>].can_buy.allow"><?php echo __('购买','b2'); ?></a>
    ...
    <div class="lottery-desc">
        <p v-if="m <= 0" v-cloak>
            <span v-if="this.resData.fir === this.resData.sec && this.resData.fir !== 0" class="green"><?php echo sprintf(__('恭喜，您中奖啦！请刷新本页：%s刷新%s','b2'),'<a href="javascript:void(0)" @click="reflush()">','</a>'); ?></span>
            <span v-else class="red"><?php echo __('运气稍微差了一点，再接再厉！','b2'); ?></span>
        </p>
        <p v-else><?php echo __('两边数字相等，代表中奖！'); ?></p>
    </div>
    <div class="mg-t shop-single-action">
        <div class="shop-single-action-left">
            ...
            <div class="shop-single-imgs">
                ...
                <div class="shop-single-img-box">
                    ...
                    <?php
                        if(!empty($imgs)){
                            foreach ($imgs as $k => $v) {
                                echo '<div class="shop-box-img">
                                    <picture>
                                        <source type="image/webp" srcset="'.$v['thumb_webp'].'">
                                        <img class="b2-radius"  src="'.$v['thumb'].'"/>
                                    </picture>
                                </div>';
                            }
                        }else{
                            echo '<div class="shop-box-img" v-if="data !== \'\' && data['.$post_id.'].thumb">
                            <picture>
                                <source type="image/webp" :srcset="data['.$post_id.'].thumb_webp">
                                <img class="b2-radius"  :src="data['.$post_id.'].thumb"/>
                            </picture>
                            </div>';
                        }
                    ?>
