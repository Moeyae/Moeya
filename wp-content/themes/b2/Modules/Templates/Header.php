...
class Header{
    ...
    public static function search_form(){
        ...
        $search = '<div class="header-search-select b2-radius" v-cloak v-show="show">';
        ...
        return '<div class="top-search mobile-hidden" ref="topsearch">
            <form method="get" action="'.home_url().'" class="mobile-search-input b2-radius">
                <div class="top-search-button">
                    <a class="top-search-select" @click.stop.prevent="show = !show" href="javascript:void(0)"><span v-show="data != \'\'" v-text="data[type]">'.__('文章','b2').'</span>'.b2_get_icon('b2-arrow-down-s-line').'</a>
                    '.$search.'
                </div>
    ...
    public static function sub_menu($_arg = ''){
        ...
        if(b2_is_page_type('post')){
        ...
        }elseif(b2_is_page_type('newsflashes')){
            $arg = array(
            ...
            if($html && !$_arg && !$audit_mode){
                $html .= '<div class="top-submit">
                    <button class="empty" onclick="postPoBox.go(\''.get_post_type_archive_link('newsflashes').'?action=showbox\',\'newsflashes\')">'.b2_get_icon('b2-flashlight-line').__('发布快讯','b2').'</button>
                </div>';
            }
            ...
            if($html && !$_arg && !$audit_mode){
                $html .= '<div class="top-submit">
                    <a class="empty button" href="'.b2_get_custom_page_url('all-circles').'" target="_blank">'.b2_get_icon('b2-donut-chart-fill').__('所有圈子','b2').'</a>
                </div>';
            }
        }else{
            $arg = array(
        ...
        <div class="header-banner">
            <div class="header-banner-content wrapper">
                ...
                <div class="header-banner-right normal-banner-right">
                    ...
                    <div>
                    <a href="'.b2_get_custom_page_url('message').'">'.b2_get_icon('b2-notification-3-line').'<b class="bar-mark" v-if="count" v-cloak></b></a>
                    </div>
                </div>' : '').'
                </div>
            </div>
        ...
        $html = '<div class="top-user-box" v-if="b2token" v-cloak>
            <div class="top-user-avatar avatar-parent" @click.stop="showDropMenu">
                '.b2_get_img(array(
                    ...
                    'src_data'=>':src="!userData.avatar ? \''.B2_LOADING_IMG.'\' : userData.avatar"'
                )).'
            </div>
            <div :class="[\'top-user-box-drop jt b2-radius\',{\'show\':showDrop}]" v-cloak>
                <div class="top-user-info-box" v-if="role.user_data">
                    <div class="top-user-info-box-name">
                        ...
                        <div class="login-out user-tips" data-title="'.__('退出登陆','b2').'"><a href="javascript:void(0)" @click="out">'.b2_get_icon('b2-login-circle-line').'</a></div>
                    </div>
                    ...
                    <div class="user-w-gold">
                        <div class="user-money user-tips" data-title="'.__('余额','b2').'"><a href="'.b2_get_custom_page_url('gold').'" target="_blank"><i>'.B2_MONEY_SYMBOL.'</i>{{role.user_data.money}}</a></div> 
    ...
    public static function user_dmsg(){
        $html = '<div class="user-tools-item dmsg-icon" @click.stop="showDmsgBox()" onselectstart="return false;">
            <span :class="dmsg.count ? \'opt-1\' : \'\'">'.b2_get_icon('b2-notification-3-line').'<b v-show="dmsg.count"></b></span>
