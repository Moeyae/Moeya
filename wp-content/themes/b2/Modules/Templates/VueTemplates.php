...
class VueTemplates{
    ...
    public static function scripts_init(){
        ...
        wp_localize_script( 'vue', 'b2_global',array(
            'is_home'=>is_home(),
            ...
            //'mark'=>$mark,
            'empty_page'=>B2_EMPTY,
            'default_video_poster'=>b2_get_option('template_single','default_video_poster'),
            'default_video_logo'=>b2_get_option('template_single','default_video_logo'),
            ...
            'poster_data'=>FileUpload::get_poster_data($post_id),
                    ...
                    <form @submit.stop.prevent="loginSubmit">
                        ...
                        <div class="login-box-in" v-show="!(invitation != 0 && (loginType == 2 && !invitationPass))">
                            ...
                            <div class="login-bottom">
                                ...
                                <button :class="locked ? \'b2-loading\' : \'\'" v-if="loginType == 2 && allowRegister == 1" :disabled="locked">'.__('快速注册', 'b2').'</button>
                        ...
                        <div class="site-terms">
                            <span>'.sprintf(__('注册登录即表示同意 %s用户协议%s、%s隐私政策%s','b2'),'<a href="'.$site_terms.'" target="_blank">','</a>','<a href="'.$site_privacy.'" target="_blank">','</a>').'</span>
                        </div>
                    </form>
                </div>
            </div>
    ...
    public static function page_nav(){
        ...
        <div class="ajax-pagenav" v-if="type === \'p\'">
            <div class="btn-group">
                ...
                <label class="pager-center" v-show="pages >= 7 || mobile"><input type="text" :value="cpaged" @keyup.enter="jump($event)" autocomplete="off"/>/{{pages}}'.__(' 页','b2').'</label>
            </div>
            <div class="btn-pager">
        ...
        <div class="ajax-more" v-else>
            <label class="pager-center" for="page-input">
                ...
                <button class="text button mar10-l" @click="jump($event)">'.__('前往','b2').'</button>
            </label>
                    ...
                    <div class="modal-content b2-radius" ref="poster">
                        ...
                        <div class="poster-content">
                            ...
                            <div class="poster-share">
                                <div class="share-text" v-if="isWeixin" v-cloak>'.__('长按分享给朋友，或者保存到手机','b2').'</div>
                                <template v-else v-cloak>
                                    ...
                                    <div class="poster-down-load">
                                        <p>下载海报：</p>
                                        <a :href="poster" download="'.$arg['title'].'.jpg" class="poster-share-download button" >'.__('点击下载','b2').'</a>
                                    </div>
                                </template>
                            </div>
                        </div>
    ...
    public static function search_box(){
        ...
        return '<div :class="[\'modal\',{\'show-modal\':show}]" v-cloak @click.stop="">
                <div class="modal-content search-box-content b2-radius" >
                    ...
                    <form @click.stop="" method="get" action="'.home_url().'" :class="showSearch ? \'search-form-normal b2-show b2-radius\' : \'search-form-normal b2-hidden b2-radius\'" v-cloak>
            ...
            <div :class="[\'modal\',\'pay-box\',{\'show-modal\':show}]">
                <div class="modal-content b2-radius">
                    ...
                    <div class="pay-type">
                        ...
                        <div class="pay-yue">'.__('余额支付','b2').'</div>
                    </div>
                </div>
            </div>
        ';
            ...
            <div :class="[\'modal\',\'ds-box\',{\'show-modal\':show}]">
                <div class="modal-content b2-radius">
                    <div class="pay-box-title">
                        <div :class="\'pay-box-left\'+(showtype == \'ds\' ? \' ds-pay-title\' : \'\')" ref="dstitle">
                            <template v-if="showtype == \'ds\'">
                                ...
                                <span>'.__('给','b2').'{{author.name}}'.__('买糖','b2').'</span>
                            </template>
                    ...
                    <div v-if="showtype == \'cg\'" class="cg-info">
                        '.B2_MONEY_SYMBOL.sprintf(__('%s可购买%s积分，最低兑换额度%s'),'<span>{{payMoney}}</span>','<span>{{creditAdd()}}</span>',B2_MONEY_SYMBOL.'<span>{{cg.min}}</span>').'
                    </div>
                    <div :class="\'pay-box-content\'+\' \'+showtype">
                        ...
                        <div class="pay-my-money" v-if="login">
                            <span class="b2-radius">'.__('您当前的余额为','b2').B2_MONEY_SYMBOL.'{{user.money ? user.money : 0}}<a href="'.home_url('/gold').'" class="b2-color" target="_blank" v-show="showtype != \'cz\' && showtype != \'card\'">'.__('充值余额','b2').'</a></span>
                            <p v-show="payMoney === 0 && payMoney != \'\' && showtype !== \'cz\' && showtype !== \'card\'" v-cloak>'.__('商品价格为0元，请使用余额付款！','b2').'</p>
                        </div>
                        <div class="pay-type">
                            <ul>
                                ...
                                <li v-if="allow.balance && login"><button class="b2-radius" :class="payType == \'balance\' ? \'picked\' : \'\'" @click="chosePayType(\'balance\')"><i class="ds-pay-yue">'.B2_MONEY_SYMBOL.'</i><span>'.__('余额','b2').'</span></button></li>
                        ...
                        <div class="pay-button">
                            <div>
                                <a v-if="jump == \'jump\'" :href="!disabled() ? href : \'javascript:void(0)\'" :class="\'button \'+(disabled() ? \'disabled\' : \'\')" :target="!disabled() ? \'_blank\' : \'\'" @click="pay()"><span v-if="!payMoney && payType != \'card\'">'.__('请输入金额','b2').'</span><span v-else-if="!payType">'.__('请选择支付方式','b2').'</span><span v-else-if="payType">'.__('支付','b2').'</span></a>
            ...
            <div class="modal-content b2-radius">
                ...
                <div class="pay-box-content">
                    ...
                    <div class="scan-info" v-else>
                        ...
                        <div class="qrcode-desc" v-else>'.__('请打开手机使用').'<span v-if="data.pay_type == \'alipay\'" class="scan-alipay">'.__('支付宝','b2').'</span><span v-else class="scan-wecatpay">'.__('微信','b2').'</span>'.__('扫码支付','b2').'</div>
                        <div class="qrcode-time">「<span v-html="timesec"></span>」</div>
                    </div>
                </div>
            </div>
                ...
                <div class="modal-content b2-radius">
                    ...
                    <div class="mp-box-content">
                        ...
                        <div v-show="!token">
                            ...
                            <p>'.__('打开微信扫一扫','b2').'</p>
                            <p class="desc">'.__('扫码并「关注我们的公众号」安全快捷登录','b2').'</p>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <span class="pay-close" @click="close()">×</span>
                        ...
                        <template v-else>
                            <label class="login-form-item">
                                ...
                                <span>'.$type.'</span> 
                                <p class="login-box-des">'.__('字母或数字的组合','b2').'</p>
                            ...
                            <label class="login-form-item">
                                ...
                                <p class="login-box-des">'.__('最少6位字符').'</p>
                            </label>
