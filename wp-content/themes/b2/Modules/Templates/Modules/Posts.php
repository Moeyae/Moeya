...
class Posts{
    ...
    public static function post_1($data,$i,$return = false){
        ...
        foreach ($post_data as $k => $v) {
            ...
            if($v['status'] === 'pending' || $v['status'] === 'draft'){
                $v['link'] = b2_get_custom_page_url('write').'?id='.$v['id'];
            }
    ...
    public static function post_2($data,$i,$return = false){
        ...
        foreach ($post_data as $k => $v) {
            ...
            //文章列表
            $html .= '<li class="grid-item post-list-item">
                <div class="item-in box b2-radius">
    ...
    public static function post_4($data,$i,$return = false){
        ...
        foreach ($post_data as $k => $v) {
            ...
            if($data['post_row_count']-1 > 0){
                $r = round(1/($data['post_row_count']-1),6)*100;
            }
            ...
            //文章列表
            $html .= '<li class="post-list-item '.($__i == 1 ? 'post-4-parent-li' : 'post-4-child-li').'" style="'.($__i == 1 ? 'width:100%!important' : 'width:'.$r.'%').'">
                <div class="item-in box b2-radius">    
    ...
    public static function post_6($data,$i,$return = false){
        ...
        foreach ($post_data as $k => $v) {
            ...
            //文章列表
            $html_in .= '<tr class="grid-item post-list-item">
                    '.$date.'
    ...
    public static function get_cats($data,$type,$i,$_pages,$index){
        ...
        if(!empty($terms)){
            $html .= '<a class="cat-list post-load-button post-load-button-more" href="'.home_url('cat-group/'.$i).'" target="_blank"><span data-type="cat">'.__('更多','b2').b2_get_icon('b2-arrow-right-s-line').'</span></a>';
        }
    ...
    public static function get_post_data($data){
        ...
        if ( $the_query->have_posts() ) {
            ...
            while ( $the_query->have_posts() ) {
                ...
                $post_data[] = apply_filters('b2_get_post_meta', $the_query->post->ID,$data);
            }
            wp_reset_postdata();
        }
    ...
    public static function get_post_metas($post_id,$data = ''){
        ...
        if(!isset($thumb_url[0]) || !$thumb_url[0]){
            $thumb_url = array(
                \B2\Modules\Common\Post::get_post_thumb(),
                400,
                300
            );
        }
        ...
        $post_meta['des'] = Sliders::get_des(0,60,$excerpt ? $excerpt : get_the_content($post_id));
