...
class Search{
    public function init($data,$i){
        ...
        if(!empty($cats)){
            ...
            $options .= '<li @click="picked(\'\',\''.__('全部','b2').'\',\''.esc_url(home_url()).'\')" :class="category == \''.__('全部','b2').'\' ? \'picked b2-radius\' : \' b2-radius\'">'.__('全部','b2').'</li>';
            foreach ($cats as $k => $v) {
        ...
        if($data['search_key']){
            ...
            foreach ($key as $k => $v) {
                $v = trim($v, " \t\n\r\0\x0B\xC2\xA0");
                $_key .= '<li class="search-key"><a :href="\''.home_url('/?s='.$v).'\'" target="_blank" style="color:'.$data['search_color'].'">'.$v.'</a></li>';
            }
            $_key .= '</ul>';
        }
