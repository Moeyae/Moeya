...
class Download extends \WP_Widget{
	...
	public static function get_widget( $atts ) {
        ...
        $count = count(get_post_meta($post_id,'b2_single_post_download_group',true));
		...
		$html = '
            ...
            <div class="wdlist" ref="wdlist">
                <div class="w-d-list" v-for="(item,index) in list" :key="index">
                    ...
                    <div class="w-d-meta">
                        <p v-for="(m,_index) in item.attrs" :key="_index" v-if="_index <= 3">
                    ...
                    <div :class="\'w-d-role b2-radius \' + (item.current_user.can.allow ? \'d-allow\' : \'\')">
                        ...
                        <div class="w-d-current can-down" v-else>
                            <span v-if="item.current_user.can.type == \'allow_all\'">
                                '.sprintf(__('今日还有%s次免费下载','b2'),'<b v-text="item.current_user.can.count"></b>').'
                            </span>
                            <span v-else>'.__('已取得下载权限','b2').'</span>
                        </div>
                    </div>
                    <div class="w-d-download">
