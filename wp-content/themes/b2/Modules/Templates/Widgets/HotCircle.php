...
class HotCircle extends \WP_Widget{
	...
	public function __construct() {
		parent::__construct(
			...
			array(
				...
				'description' => __( '“热门圈子”小工具', 'b2' ),
			)
		);

		//默认设置项
		self::$defaults = array(
            'circle_title'=>__('热门圈子','b2'),
            'circle_count'=>6,
			'show_mobile'=>0
		);
	...
	public static function get_widget( $atts ) {
			...
			<div class="hot-circle-widget" id="hot-circle-widget" ref="hotCircle" data-count="'.$instance['circle_count'].'">
				<div class="hot-circle-widget-content">
                        ...
                        <ul>
                            <li v-for="item in data">
                                ...
                                <div class="widget-circle-info">
									...
									<div class="widget-circle-meta">
										<span><b v-text="item.user_count"></b>'.__('个圈友').'</span>
										<i>•</i>
                                        <span><b v-text="item.topic_count"></b>'.__('个话题').'</span>
                                    </div>
								</div>
                    ...
                    <div v-else-if="data !== \'\'" class="widget-circle-none" v-cloak>
                        <span>'.sprintf(__('没有%s圈子','b2'),'<b v-text="type === \'hot\' ? \''.__('热门','b2').'\' : (type === \'join\' ? \''.__('加入','b2').'\' : \''.__('创建','b2').'\')"></b>').'</span>
					</div>
					<div class="widget-mission-footer b2-color"><a href="'.b2_get_custom_page_url('all-circles').'" target="_blank">'.__('所有圈子','b2').'</a></div>
				</div>
			</div>
		...
		if ( empty( $widget )) {
            ...
            $widget .= '<h2>
            <button :class="type === \'hot\' ? \'b2-radius picked box\' : \'b2-radius box\'" @click="getCirclesList(\'hot\')">'.__('热门圈子','b2').'</button>
