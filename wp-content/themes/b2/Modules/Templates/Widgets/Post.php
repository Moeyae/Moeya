...
class Post extends \WP_Widget{
	...
	public function __construct() {
		...
		$this->cmb2_fields = array(
			...
			array(
				'name'    => __('分类ID','b2'),
				'desc'    => sprintf(__('可以填写分类ID，专题ID，标签ID等，多个请用竖线|隔开。ID之间是或者的关系，比如设置ID为1的标签和ID为12的分类，写作%s，这里将会显示标签是1，或者分类是12的文章。留空将显示全部。','b2'),'<code>1|12</code>'),
				'id_key'  => 'post_cat',
				'id'      => 'post_cat',
				'type'    => 'textarea_small'
			),
	...
	public static function get_widget( $atts ) {
		...
		if($instance[ 'days' ]){
            $args['date_query'] = array(
                array(
                    'after'     => date('Y-m-d',strtotime("-".$instance[ 'days' ]." days")),//7天的时间
                    'inclusive' => true,
                )
            );
        }
