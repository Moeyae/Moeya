	...
	public function __construct() {
		parent::__construct(
			...
			array(
				...
				'description' => __( '“推荐圈子”小工具', 'b2' ),
			)
		);

		//默认设置项
		self::$defaults = array(
			'about_title'=>__('推荐圈子','b2'),
            'circle_ids'  => '',
			'show_mobile'=>0
		);
		...
		$this->cmb2_fields = array(
            ...
            array(
				...
				'id'     => 'circle_ids',
				'type'   => 'textarea_small',
				'desc'=>__('请直接填写要显示的圈子ID，每个ID占一行，推荐只在圈子页面使用以达到最好的显示效果。','b2')
            ),
			array(
				'name'=>__('移动端是否显示','b2'),
				'id_key'=>'show_mobile',
		...
		$html = '
			<div class="recommended-widget">
				<ul class="recommended-circle-gujia" ref="recommendedGujia" data-ids=\''.json_encode($arr).'\'>
					'.($li === '' ? '<div class="b2-pd b2-b-t fs12">'.__('请前往小工具中设置推荐圈子','b2').'</div>' : $li).'
				</ul>
