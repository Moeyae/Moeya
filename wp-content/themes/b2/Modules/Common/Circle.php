...
class Circle{
    public function init(){
        //添加圈子的类型
        add_action( 'init', array($this,'create_circle_taxonomies'),0);
    }

    //添加圈子的文章类型
    public function create_circle_taxonomies() {
        $name = __('圈子','b2');
        $labels = array(
            'name'              => $name,
            'singular_name'     => $name,
        ...
        $args = array(
            ...
            'query_var'         => true,
            'show_in_rest' => true,
            'rewrite'           => array( 'slug' => 'circle','with_front' => false ),
        );
    ...
    public static function insert_topic_card($id){
        ...
        if(!$post_type && strpos($url,'/circle') === false) return array('error'=>__('不支持此类连接','b2'));
    ...
    public static function check_topic_role($user_id,$circle_id = 0,$circles = true){
        ...
        $allow_pendings = b2_get_option('circle_topic','topic_pending_count',$circle_id);
    ...
    public static function join_circle($circle_id){
        ...
        //检查入圈权限
        if(apply_filters('b2_is_user_in_circle',$user_id,$circle_id)) return array('error'=>__('您已经加入了此圈子','b2'));
    ...
    public static function get_circle_data($circle_id,$current_circle_id = 0){
        ...
        if(!isset($c->count)) return array('error'=>__('不存在该圈子','b2'));
    ...
    public static function get_circle_admin_data($circle_id){
        ...
        if(!empty($res)){
            ...
            return User::get_user_public_data($res['user_id'],true);
        }

        return User::get_user_public_data(1,true);
    }
    ...
    public static function is_circle_admin($user_id,$circle_id){
        ...
        return CircleRelate::isset(array(
            'user_id'=>$user_id,
            'circle_id'=>$circle_id,
            'circle_role'=>'admin'
        ));
    ...
    public static function insert_circle_topic($data){
        ...
        if(!$data['circle']) return array('error'=>__('请选择话题所在的圈子'));
        ...
        if(!$role['topic_type_role'][$topic_type]) return array('error'=>__('权限不足','b2'));

        if(!$role['in_circle']) return array('error'=>__('您没有权限在此圈子发布话题','b2'));
        ...
        $title = wp_strip_all_tags(sanitize_text_field($title));

        if(b2getStrLen($title) > 200) return array('error'=>__('标题太长，请限制在1-200个字符之内','b2'));

        if($topic_type === 'guess' && $title == ''){
            return array('error'=>__('你猜功能必须要设置一个标题','b2'));
        }

        //话题内容
        $content = str_replace(array('{{','}}'),'',$data['content']);
        $content = wp_strip_all_tags(sanitize_textarea_field($content));
        ...
        if(empty($data['title'])){
            $data['title'] = mb_strimwidth($content,0,50,'','utf-8');
        ...
        if(isset($data['topic_id']) && (int)$data['topic_id']){
    ...
    public static function save_topic_role($post_id,$data){
        ...
        switch ($data['role']['see']) {
            ...
            case 'comment':
            case 'login':
                update_post_meta($post_id,'b2_topic_read_role',$data['role']['see']);
        ...
        if($data['role']['currentCircle'] === 'true'){
            update_post_meta($post_id, 'b2_currentCircle', 1);
        }else{
    ...
    public static function get_topic_attachment($post_id){
        ...
        if(!empty($video)){
            ...
            foreach ($video as $k => $v) {
                ...
                $w = 390;
                if(isset($video_data['width']) && isset($video_data['height'])){
                    $h = round(($w*$video_data['height'])/$video_data['width']);
                }else{
                    $h = round(390/16*9);
                }
                

                if(isset($video_data['width']) && $video_data['width'] < 390){
                    $w = $video_data['width'];
                    $h = $video_data['height'];
                    
                    if($h > 500){
                        $h = 500;
                        $w = round(($h*$video_data['width'])/$video_data['height']);
                    }
                }
                ...
                foreach ($files['list'] as $_k => $_v) {
                    if($k === 'file'){
                        if(get_post_type($_v['id'])){
                            $list[$k][] = array(
                                'id'=>(int)$_v['id'],
                                'size'=>sanitize_text_field($_v['size']),
                                'ext'=>sanitize_text_field($_v['ext']),
                                'name'=>sanitize_text_field($_v['name'])
                            );
                        }
                    }else{
                        if(get_post_type($_v['id'])){
    ...
    public static function check_ask($data){
        ...
        if(count((array)$ask['pickedList']) > 6) return array('error'=>__('指定的用户过多','b2'));
        ...
        if($ask['reward'] === 'money'){
            ...
            if((float)$ask['pay'] > (float)$money) return array('error'=>__('余额不足','b2'));
        }

        $end_time = date('Y-m-d H:i:s',strtotime('+'.$ask['time'].' day'));
    ...
    public static function create_circle($data){
        ...
        if(!$role && !user_can( $user_id, 'manage_options' )) return array('error'=>__('您没有权限创建圈子','b2'));
        ...
        if(!$data['tags']) return array('error'=>__('请选择圈子类别','b2'));
        ...
        if($data['pay']['type'] === 'money'){
            ...
            if($empty) return array('error'=>__('请填写入圈所需金额','b2'));
        }

        if($data['pay']['type'] === 'lv'){
            if(empty($data['role']['lv'])) return array('error'=>__('请选择允许入圈的用户组','b2'));
        ...
        if($data['read'] !== 'public' && $data['read'] !== 'private') return array('error'=>__('请设置圈子隐私','b2'));

        if(!$data['info']['icon'] || !$data['info']['name'] || !$data['info']['desc']){//|| !$data['info']['cover']
            return array('error'=>__('请完善圈子资料','b2'));
        }

        if(!preg_match('/.*(\.png|\.jpg|\.jpeg|\.gif)$/', $data['info']['icon'])){
            return array('error'=>__('圈子图标错误','b2'));
        }
        ...
        $data['info']['name'] = str_replace(array('{{','}}'),'',sanitize_textarea_field($data['info']['name']));
        $data['info']['desc'] = str_replace(array('{{','}}'),'',sanitize_textarea_field($data['info']['desc']));

        if(b2getStrLen($data['info']['name']) < 2 || b2getStrLen($data['info']['name']) > 20){
            return array('error'=>__('圈子名称必须大于2个字符，小于10个字符','b2'));
        }

        if(!$data['info']['slug']) return array('error'=>__('请填写圈子英文名称','b2'));

        $mb = mb_strlen($data['info']['slug'],'utf-8');
        $st = strlen($data['info']['slug']);
        if($st !== $mb) return array('error'=>__('请使用纯英文','b2'));

        if(get_term_by('slug',$data['info']['slug'], 'circle_tags')) return array('error'=>__('圈子英文名称有重复，请更换英文名称','b2'));

        if(get_term_by('name',$data['info']['name'], 'circle_tags')) return array('error'=>__('圈子名称有重复，请更换名称','b2'));

        if(b2getStrLen($data['info']['desc']) < 10 || b2getStrLen($data['info']['desc']) > 100){
            return array('error'=>__('圈子简介必须大于10个字符，小于10个字符','b2'));
        }
    ...
    public static function check_best($data){
        ...
        if(empty($users)){
            ...
            if($data['reward'] === 'credit'){
                $total = Credit::credit_change($asker,$data['pay']);
            }else{
                $total = User::money_change($asker,$data['pay']);
            }
    
            Message::add_message(array(
                'user_id'=>$asker,
                'msg_type'=>$data['reward'] === 'money' ? 84 : 83,
                'msg_read'=>1,
                'msg_date'=>current_time('mysql'),
                'msg_users'=>'',
                'msg_credit'=>$data['pay'],
                'msg_credit_total'=>$total,
                'msg_key'=>$data['topic_id'],
                'msg_value'=>''
            ));
        ...
        foreach ($users as $v) {
            if($data['reward'] === 'credit'){
                $total = Credit::credit_change($v,$pay_one);
            }else{
                $total = User::money_change($v,$pay_one);
            }
    
            Message::add_message(array(
                'user_id'=>$v,
                'msg_type'=>$data['reward'] === 'money' ? 82 : 81,
                'msg_read'=>1,
                'msg_date'=>current_time('mysql'),
                'msg_users'=>$asker,
                'msg_credit'=>$pay_one,
    ...
    public static function get_data_by_topic_id($topic_id,$user_id = 0,$topic_query = null){
        ...
        $author_id = $topic_query ? $topic_query->post->post_author : get_post_field ('post_author', $topic_id);

        if(!$author_id) return array();

        $author_data = User::get_user_public_data($author_id,true);

        $circle = wp_get_object_terms($topic_id,'circle_tags');
        if(empty($circle)) return array('error'=>__('不存在此圈子','b2'));
        $circle = $circle[0];
        ...
        if($allow_read['allow']){
            $content = sanitize_textarea_field(get_post_field('post_content', $topic_id));
        ...
        return array(
            ...
            'sticky'=>in_array($topic_id,$stickys) ? 1 : 0
        );
    }
    ...
    public static function get_circles_list($data){
        ...
        $arg['exclude'] = get_option('b2_circle_default');
        ...
        if(!empty($terms)){
            ...
            foreach ($terms as $k => $v) {
                $icon = b2_get_thumb(array('thumb'=>get_term_meta($v->term_id, 'b2_circle_icon', true),'width'=>300,'height'=>300));
                $list[] = array(
                    'id'=>$v->term_id,
                    'name'=>$v->name,
                    'desc'=>$v->description ? $v->description : __('没有描述','b2'),
                    'admin'=>self::get_circle_admin_data($v->term_id),
                    'link'=>get_term_link($v->term_id),
                    'in_circle'=>apply_filters('b2_is_user_in_circle',$user_id,$v->term_id),
                    'user_count'=>$arg['exclude'] == $v->term_id ? User::user_count() : self::user_count_in_circle($v->term_id),
                    'circle_count'=>$circle_count,
                    'topic_count'=>$v->count,
                    'icon'=>$icon,
                    'icon_webp'=>apply_filters('b2_thumb_webp',$icon),
                    //'cover'=>b2_get_thumb(array('thumb'=>get_term_meta($v->term_id, 'b2_circle_cover', true),'width'=>$size['w'],'height'=>$size['h']))
                );
            }
        }
    ...
    public static function get_circle_tags(){
        ...
        if(!$tags) return array('error'=>__('请设置圈子标签','b2'));
    ...
    public static function get_all_circles($tag = '',$paged = 1){
        $tags = self::get_circle_tags();
        if(!$tags) return array('error'=>__('没有圈子类别','b2'));
    ...
    public static function user_can_delete_post($post_id,$user_id){
        ...
        $m = round((strtotime(current_time('Y-n-j G:i:s')) - strtotime($post_date)) / 60);
        ...
        if($m >= $edit_time){
            return array('error'=>__('已过期，无法删除，请联系圈主或管理员','b2'));
        }
    ...
    public static function delete_topic($topic_id){
        ...
        return wp_delete_post($topic_id,true) ? true : false;
    }
    ...
    public static function delete_answer($answer_id){
        ...
        if(isset($user_can_delete['error'])){
            if(!$role['is_circle_admin'] || !$role['is_admin']){
                return array('error'=>__('无权删除答案'));
            }
        }

        return wp_delete_post($answer_id,true) ? true : false;
    }

    public static function get_circle_data_by_circle_ids($ids){
        $data = array();
        if(empty($ids)) return array('error'=>__('请指定圈子的ID','b2'));
        foreach ($ids as $k => $v) {
        ...
        if($circle_id == get_option('b2_circle_default')){
            ...
            if(!empty($users_found)){
                foreach ($users_found as $k => $v) {
                    $data['list'][] = array(
                        'id'=>$v->ID,
                        'user_data'=>User::get_user_public_data($v->ID,true),
            ...
            if(!empty($res)){
                foreach ($res as $k => $v) {
                    $data['list'][] = array(
                        'id'=>$v['user_id'],
                        'user_data'=>User::get_user_public_data($v['user_id'],true),
                        'date'=>$v['join_date'],
                        'role'=>$v['circle_role'],
    ...
    public static function submit_topic_answer($data){
        ...
        $content = sanitize_textarea_field(str_replace(array('{{','}}'),'',$data['content']));
        ...
        if($data['id']){
            ...
            if(isset($user_can_delete['error']) && !$is_admin && !$is_circle_admin){
                return array('error'=>__('已过期，无法编辑'));
            }
        ...
        if($post_id){
            if($data['image']){
                update_post_meta($post_id,'b2_circle_answer_image',$data['image']);
            }

            if($data['file']){
                update_post_meta($post_id,'b2_circle_answer_file',$data['file']);
            }
    ...
    public static function get_answer_by_id($answer_id,$user_id){
        ...
        $can_edit = self::user_can_delete_post($answer_id,get_current_user_id());
        ...
        $data = array(
            'id'=>$answer_id,
            'content'=>sanitize_textarea_field(get_post_field('post_content', $answer_id)),
            'user'=>User::get_user_public_data($author,true),
            'date'=>Post::time_ago($date),
            'full_answer'=>false,
    ...
    public static function answer_right($answer_id){
        ...
        if($pay_type === 'credit'){
            $total = Credit::credit_change($answer,$pay);
        }else{
            $total = User::money_change($answer,$pay);
        }

        Message::add_message(array(
            'user_id'=>$answer,
            'msg_type'=>$pay_type === 'credit' ? 79 : 80,
            'msg_read'=>0,
            'msg_date'=>current_time('mysql'),
            'msg_users'=>$user_id,
            'msg_credit'=>$pay,
    ...
    public static function get_edit_data($topic_id){
        ...
        return array(
            ...
            'ask'=>array(
                ...
                'userList'=>$ask_users,
                'reward'=>get_post_meta($topic_id,'b2_circle_ask_reward',true),
                'time'=>$ask_time ? round((strtotime($ask_time) - strtotime(current_time('Y-n-j G:i:s'))) / 86400) : ''
            ),
            'vote'=>array(
            ...
            'title'=>get_post_meta($topic_id,'b2_auto_title',true) ? '' : get_the_title($topic_id),
