...
class CircleRelate{
    ...
    public static function get_count($arg){
        ...
        if(isset($arg['user_id']) && $arg['user_id'] !== ''){
            $where .= sprintf(' AND `user_id`=%d',$arg['user_id']);
        }

        if(isset($arg['circle_id']) && $arg['circle_id'] !== ''){
            $where .= sprintf(' AND `circle_id`=%d',$arg['circle_id']);
        }

        if(isset($arg['circle_role']) && $arg['circle_role'] !== ''){
            $where .= sprintf(' AND `circle_role`="%s"',$arg['circle_role']);
        }

        if(isset($arg['circle_key']) && $arg['circle_key'] !== ''){
            $where .= sprintf(' AND `circle_key`="%s"',$arg['circle_key']);
        }

        if(isset($arg['circle_value']) && $arg['circle_value'] !== ''){
            $where .= sprintf(' AND `circle_value`="%s"',$arg['circle_value']);
        }
