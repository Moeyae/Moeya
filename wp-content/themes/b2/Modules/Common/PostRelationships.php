...
class PostRelationships{
    ...
    public static function get_count($arg){
        ...
        if(isset($arg['type']) && $arg['type']){
            $where .= sprintf(' AND `type`="%s"',$arg['type']);
        }

        if(isset($arg['post_id']) && $arg['post_id'] !== ''){
            $where .= sprintf(' AND `post_id`=%d',(int)$arg['post_id']);
        }

        if(isset($arg['comment_id']) && $arg['comment_id'] !== ''){
            $where .= sprintf(' AND `comment_id`=%d',(int)$arg['comment_id']);
        }

        if(isset($arg['user_id']) && $arg['user_id'] !== ''){
            $where .= sprintf(' AND `user_id`=%d',(int)$arg['user_id']);
        }

        if(isset($arg['k']) && $arg['k'] !== ''){
            $where .= sprintf(' AND `k`="%s"',$arg['k']);
        }

        if(isset($arg['v']) && $arg['v'] !== ''){
            $where .= sprintf(' AND `v`="%s"',$arg['v']);
        }
        ...
        if(isset($arg['type'])){
            wp_cache_set($cache_key,'i_'.$count,'b2_relate_count_'.$arg['type'],24 * HOUR_IN_SECONDS);
        }
    ...
    public static function get_data($arg){
        ...
        if(isset($arg['type'])){
            wp_cache_set($cache_key,$res,'b2_relate_data_'.$arg['type'],24 * HOUR_IN_SECONDS);
        }
