...
class FileUpload{
    ...
    public static function get_poster_data($post_id){
        ...
        if(!isset($default_poster_settings['single_poster_group'][0])){
            $default_poster_settings = array(
                ...
                'single_poster_default_text'=>wptexturize(esc_attr(b2_get_option('template_single','single_poster_default_text') )),
                'single_poster_default_desc'=>wptexturize(esc_attr(b2_get_option('template_single','single_poster_default_desc') )),
        ...
        $title = wptexturize(trim($title, " \t\n\r\0"));

        //文章描述
        $text = b2_get_des($post_id,60);
        $text = wptexturize(sanitize_textarea_field($text));
        ...
        //$thumb_url = b2_get_thumb(array('thumb'=>$thumb_url,'height'=>625,'width'=>780));
    ...
    public static function file_upload($request){
        ...
        return array('error'=>sprintf(__('上传失败(%s)','b2'),$_FILES['file']['name']));
    }
}