...
class Login{
    ...
    public static function get_user_info($ref){
        ...
        //关联分销
        if($ref){
            $distribution = apply_filters('b2_check_distribution', $current_user_id,$ref);
        }
    ...
    public static function send_email_code($code,$email){
        ...
        if(!$send){
            return array('error'=>__('验证码发送失败，请联系管理员'));
        }
    ...
    public static function luosimao($code){
        ...
        if($res['error'] === 0){
            ...
            return array('error'=>__('验证码错误代号：').$res['error']); 
        }
    ...
    public static function check_nickname($nickname){
        ...
        if (!preg_match("/^[\x{4e00}-\x{9fa5}A-Za-z0-9]+$/u", $nickname)) {
            return array('error'=>__('昵称中包含特殊字符，请重新填写','b2'));
        }
        ...
        if($result){
            unset($result);
            return array('error'=>__('昵称已被注册了，请重新填写','b2'));
        }
    ...
    public static function forgot_pass($request){
        ...
        if(!is_email($request['username']) && !self::is_phone($request['username'])){
            return array('error'=>__('需要您绑定了邮箱或者手机号码方可找回密码'));
        }
