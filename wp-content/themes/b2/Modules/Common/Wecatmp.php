    ...
    public static function event($data,$api){
        ...
        if($data['Event'] === 'subscribe' && isset($data['EventKey']) && !empty($data['EventKey'])){
            ...
            if((int)$sence_id === 1){
                ...
                $api->text(sprintf(__('感谢您关注「%s」，%s','b2'),get_bloginfo('name'),current_time('mysql')))->reply();
            ...
            if(isset($user_info['nickname'])){
                $user_info['avatar'] = str_replace('http://','https://',$user_info['headimgurl']);
        ...
        if(isset($data['EventKey']) && get_transient('sence_id_'.$data['EventKey'])){
            ...
            $api->text(sprintf(__('欢迎登录「%s」，%s','b2'),get_bloginfo('name'),current_time('mysql')))->reply();
        ...
        if ( $the_query->have_posts() ) {
            ...
            while ( $the_query->have_posts() ) {
                ...
                $res[] = array(
                    ...
                    'Description'=>Sliders::get_des($the_query->post->ID,150),
                    'PicUrl'=>b2_get_thumb(array('thumb'=>Post::get_post_thumb($the_query->post->ID),'width'=>360,'height'=>200)),
                    'Url'=>$key ? get_the_permalink($key[0]['post_id']) : home_url('/?s='.$data['Content'].'&type=post')
                );
            }
    ...
    public static function mp_login($code){
        ...
        if($check){
            ...
            if($invitation == 1 || $invitation == 2){
                ...
                return OAuth::create_user($user_info,$check_invitation,true);
            }
        }
    }
