...
class RestApi{
    ...
    public static function commentVote($request){
        return Comment::comment_vote($request['type'],$request['comment_id']);
    }
    ...
    public static function invitationCheck($request){
        ...
        if(isset($res['error'])){
            ...
            return new \WP_REST_Response(array('msg'=>__('邀请码可用')),200);
        }
    ...
    public static function sendCode($request){
        if(!b2_check_repo($request['token'])) return array('error'=>__('请不要重复提交','b2'));
    ...
    public static function socialLogin($request){
        $res = OAuth::init($request['type'],$request['code']);
            ...
            foreach ($res['data'] as $k => $v) {
                $html .= '<li>
                    <div class="following-avatar">
                        <img src="'.$v['avatar'].'" class="avatar b2-radius" />
                    </div>
    ...
    public static function checkFollowing($request){
        $res = User::check_following($request['user_id'],$request['post_id']);
        ...
        if(isset($res['error'])){
            ...
            foreach ($res['data'] as $k => $v) {
                $html .= '<li>
                    ...
                    <img src="'.b2_get_thumb(array('thumb'=>$v['thumb'],'type'=>'fill','width'=>'120','height'=>'74')).'" class="avatar b2-radius" />
                </div>' : '').'
                    <div class="following-info">
    ...
    public static function saveQrcode($request){
        if(!b2_check_repo()) return array('error'=>__('请不要重复提交','b2'));
    ...
    public static function saveAddress($request){
        if(!b2_check_repo()) return array('error'=>__('请不要重复提交','b2'));
        $res = User::save_address($request['address'],$request['name'],$request['phone'],$request['user_id'],$request['key']);
    ...
    public static function balancePay($request){
        if(!b2_check_repo($request['order_id'])) return array('error'=>__('请不要重复提交','b2'));
    ...
    public static function BatchPayment($request){
        ...
        if(!b2_check_repo($user_id.'batch_payment')) return array('error'=>__('请不要重复提交','b2'));
    ...
    public static function getGoldList($request){
        $res = User::get_gold_list($request['user_id'],$request['type'],$request['paged']);
    ...
    public static function getMyOrders($request){
        $res = Orders::get_my_orders($request['user_id'],$request['paged']);
    ...
    public static function getShopItemsData($request){
        $res = Shop::get_shop_items_data($request['ids'],$request['return']);
    ...
    public static function getEmail($request){
        ...
        if(isset($data->user_email) && $data->user_email && strpos(home_url(),explode('@',$data->user_email)[1]) === false) {
    ...
    public static function getOrderExpress($request){
        $res = Shop::shop_get_express_data($request['id'],$request['address']);
    ...
    public static function getMyPartner($request){
        $res = Distribution::get_my_partner($request['user_id'],$request['partner_paged']);
