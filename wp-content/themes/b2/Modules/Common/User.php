...
class User{
    ...
    public static function get_letter_avatar($letter){
        ...
        return B2_THEME_URI.'/Assets/fontend/images/default-avatar.png';
        ...
        $avatar = B2_THEME_URI.'/Assets/fontend/images/default-avatar.png';
    ...
    public function get_avatar_url($user_id,$args){
        ...
        if(is_numeric($user_id)){
            ...
            $args['url'] = B2_THEME_URI.'/Assets/fontend/images/default-avatar.png';
        }
    ...
    public static function get_user_public_data($user_id,$private = false){
       ...
       wp_cache_set('b2_user_'.$user_id,$data,'b2_user_data',HOUR_IN_SECONDS);
    ...
    public static function get_author_info($user_id){
        ...
        if($current_user_id){
            ...
            $key_following = array_search($current_user_id,$following);
        }else{
            $key_following = false;
        }
        ...
        $data = array(
            ...
            'followed'=>$key_following ? true : false
        );
    ...
    public static function get_user_lv($user_id,$private = false){
        ...
        if(isset($roles[$user_lv])){     
            ...
            $next_credit = isset($roles[$_lv_next]['credit']) && (int)$roles[$_lv_next] != 0 ? (int)$roles[$_lv_next]['credit'] : (int)$roles[$user_lv]['credit'];
    ...
    public static function check_user_can_download_all($user_id){
        ...
        if(isset($user_role['vip']['lv']) && isset($roles[$user_role['vip']['lv']])){
            $vip_settings = $roles[$user_role['vip']['lv']];
            if(isset($vip_settings['allow_download']) && $vip_settings['allow_download'] == 1){

                $count = isset($vip_settings['allow_download_count']) ? (int)$vip_settings['allow_download_count'] : 20;
    ...
    public static function check_dark_room($user_id){
        ...
        if($dark_room === 1){
            ...
            if(((strtotime(current_time('Y-m-d H:i:s'))-strtotime($update_date))/86400) > $dark_days){
    ...
    public static function check_user_vip_time($user_id){
        ...
        if(isset($user_lv_time['end']) && (string)$user_lv_time['end'] !== '0' && (int)$day !== 0){
            //检查是否过期
            if($user_lv_time['end'] < strtotime(current_time('Y-m-d H:i:s'))){
    ...
    public static function get_followers($user_id,$paged,$number){
        ...
        for ($i=$offest; $i < $offest + $number; $i++) {
            if(isset($follow[$i])){
                ...
                $key_following = array_search($follow[$i],$following);
    ...
    public static function user_follow_action($user_id){
        ...
        if($key_following === false){
            ...
            if(!$task){
                ...
                $total = Credit::credit_change($current_user_id,$credit);
            }
        ...
        $key_follower = array_search($current_user_id,$follower);
    ...
    //检查是否已经关注
    public static function check_following($user_id,$post_id){
        $current_user_id = get_current_user_id();
        ...
        $post_meta = Post::get_post_data($post_id);
    ...
    public static function get_author_settings($user_id){
        ...
        return apply_filters('b2_author_settings_info', array(
    ...
    public static function un_build($type,$user_id){
        ...
        $uid = get_user_meta($user_id,'zrz_'.$type.'_uid',true);
    ...
    public static function save_desc($desc,$user_id){
        ...
        //更新描述
        update_user_meta($user_id,'description',str_replace(array('{{','}}'),'',sanitize_text_field($desc)));
    ...
    //保存收货地址
    public static function save_address($address,$name,$phone,$user_id,$key){
        $current_user_id = get_current_user_id();
    ...
    public static function cash_out($money){
        ...
        if($my_money < $money) return array('error'=>__('余额不足','b2'));
    ...
    public static function get_user_mission_data($user_id){
        ...
        if(!$mission_always){
            ...
            $days = (int)((strtotime(current_time('Y-m-d')) - strtotime(substr($mission_always,0,10)))/86400) - 1;
            $days = $days < 0 ? 0 : $days;
    ...
    public static function get_vip_info(){
        ...
        if(!empty($user_data)){
            ...
            if(isset($time['end'])){
                $time = $time['end'];
                $time = $time == 0 ? 'long' : date('Y-m-d H:i:s',$time);
                $user_data['time'] = $time;
            }
        }else{
            $user_data = 'guest';
    ...
    public static function check_user_media_role($user_id,$type){
        ...
        if($lv && isset($roles[$lv])){
            if(in_array($lv,$role)) return true;
        }

        if($vip && isset($roles[$vip])){
            if(in_array($vip,$role)) return true;
        }
        
        if(get_user_meta($user_id,'b2_title',true)){
            if(in_array('verify',$role)) return true;
        }
    ...
    public static function get_dark_room_users($paged,$type){
        ...
        if ( ! empty( $user_query->results ) ) {
            foreach ( $user_query->results as $user ) { 
                ...
                $end_date = date("Y-m-d H:i:s", strtotime($start_date.' +'.$days.' day'));

                if($end_date > current_time('mysql')){
                    $end_date = __('下次来访时','b2');
                }else{
                    $end_date = b2_timeago($end_date);
                }
