...
class Normal{
    ...
    public static function get_default_settings($key){
        $arr = array(
            ...
            'default_user_avatar'=>B2_THEME_URI.'/Assets/fontend/images/default-avatar.png',
    ...
    public function normal_options_page(){
        ...
        //限制操作频次
        $safe->add_field(array(
            'before_row'=>sprintf(__('%s限制操作频次%s%s此功能用来防止某些人通过接口批量对数据进行操作，比如批量发送垃圾信息，批量下载，批量硬解密码等操作%s%s如果您不知道下面的设置项是什么意思，请保持默认即可%s','b2'),'<h2>','</h2>','<p>','</p>','<p>','</p>'),
            'name'    => __( '记录时间段', 'b2' ),
        ...
        $safe->add_field(array(
            'before_row'=>sprintf(__('%s消息表自动清理%s%s消息表用来记录用户的消息数据，包括积分和余额记录，用户比较多的时候数据量可能会很大，如果消息表（zrz_message）数据比较庞大，比如根据自己服务器配置超过10万条或者100万条以上开启此处清理。%s','b2'),'<h2>','</h2>','<p>','</p>'),
        ...
        $user->add_field(array(
            ...
            'desc'    => sprintf(__( '加密前：%s%s加密后：%s%s设置完成以后请保存一下固定链接，前端用户需要重新登录', 'b2' ),'<code>'.home_url().'/users/<span class="red">123</span></code>','<br>','<code>'.home_url().'/users/<span class="red">pXyQNJVVV</span></code>','<br><span class="red">','</span>'),
            'options'          => array(
                1 => __( '加密', 'b2' ),
                0 => __( '不加密', 'b2' ),
            ),
        ...
        $user->add_field(array(
            'name'    => __( '重写用户页面规则', 'b2' ),
            'desc'    => sprintf(__( '如果需要用户页面是%s，则在此处填写%s%s设置完成以后请保存一下固定链接，前端用户需要重新登录', 'b2' ),'<code>'.home_url().'/<span class="red">users</span>/123</code>','<code>users</code>','<br><span class="red">','</span>'),
            'id'=>'user_slug',
            'type'             => 'text',
    ...
    public static function pay_settings(){
        ...
        $pay->add_field(array(
            ...
            'default' => '0',
            'before_row'=>'<p class="red">'.__('第三方支付不能保证100%安全，我们只对接口做兼容，其他请自行负责。','b2').'</p>'
        ));
        ...
        $pay->add_field(array(
            ...
            'id'      => 'paypal_email',
            'type'    => 'input',
            'desc'=>sprintf(__('您的收款账户，通常为email%s需要特别注意的是，如果您的收款账户为中国大陆地区，则账户类型必须是企业。并且只能收取非中国大陆区的PayPal付款。%s','b2'),'<br><span class="red">','</span>'),
