...
class Post{
    ...
    public function post_settings(){
        ...
        $post_meta->add_group_field( $video_group, array(
            ...
            'desc'=>__('用户未支付或者权限不足的情况下可以进行视频预览，如果没有支付限制，请留空（只支持类似 .mp4这种文件形式，不支持优酷等第三方）','b2'),
        ) );
        ...
        $post_meta->add_group_field( $video_group, array(
            ...
            'desc'=>__('请上传视频，或者直接输入视频地址，此为必填，否则前台无法播放视频（只支持类似 .mp4这种文件，不支持优酷等第三方）','b2'),
        ) );
    ...
    public function post_side_settings(){
        ...
        $post_gg->add_field(array(
            ...
            'id'   => 'b2_post_gg',
            'type' => 'textarea',
            'desc'=> __('支持 HTML，用户每次打开该文章都会显示此公告，如果不需要显示，请留空。','b2')
        ));
        ...
        $gg_meta->add_field(array(
            ...
            'id'   => 'b2_gg_days',
            'type' => 'input',
            'desc' => __('如果没有新的公告，弹出一次用户关闭后再隔多少天再次弹出','b2'),
            'default'=>1,
        ));

        $gg_meta->add_field(array(
            'name' => __('旧公告多少天后自动关闭弹出','b2'),
            'id'   => 'b2_gg_over',
            'type' => 'input',
            'desc' => __('如果没有新的公告发布，旧的公告多少天以后会自动失效，不再弹出。','b2'),
            'default'=>7,
        ));
    ...
    public function document_order(){
        ...
        $document->add_field(array(
            'name' => __('文档在该组别的排序','b2'),
            'id'   => 'b2_document_order',
            'type' => 'text',
            'desc' => __('排序必填，否则文档分类中不显示，数字越小，排名越靠前','b2'),
        ));
    }
}