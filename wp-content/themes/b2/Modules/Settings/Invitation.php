...
class Invitation{
    ...
    public function invitation_options_page(){
        ...
        $invitation->add_field( array(
            'name'    => __( '邀请码获取链接', 'b2' ),
            'id'      => 'invitation_text',
            'desc'=> sprintf(__( '此项内容显示在注册窗口，邀请码填写窗口格式如下：%s。%s比如：%s', 'b2' ),
            '<code>文字描述|链接</code>',
            '<br />',
    ...
    public function list_option_page_cb($cmb_options){
        ...
        <div class="wrap cmb2-options-page option-<?php echo $cmb_options->option_key; ?>">
            ...
            <div class="wrap">
                <ul class="subsubsub">
                    <li class="all"><a href="<?php echo home_url('/wp-admin/admin.php?page=b2_invitation_list&invitation_status=all'); ?>" class="<?php echo $status === 'all' ? 'current' : ''; ?>"><?php echo __('所有','b2'); ?><span class="count">（<?php echo $invitation_code->get_status_count('all'); ?>）</span></a> |</li>
                    <li class="mine"><a href="<?php echo home_url('/wp-admin/admin.php?page=b2_invitation_list&invitation_status=0'); ?>" class="<?php echo $status === '0' ? 'current' : ''; ?>"><?php echo __('未使用','b2'); ?><span class="count">（<?php echo $invitation_code->get_status_count(0); ?>）</span></a> |</li>
                    <li class="publish"><a href="<?php echo home_url('/wp-admin/admin.php?page=b2_invitation_list&invitation_status=1'); ?>" class="<?php echo $status === '1' ? 'current' : ''; ?>"><?php echo __('使用','b2'); ?><span class="count">（<?php echo $invitation_code->get_status_count(1); ?>）</span></a></li>
                </ul>
