...
class Taxonomies{
    ...
    public function register_taxonomy_metabox(){
        ...
        $collection->add_field( array(
            ...
            'id'      => 'b2_tax_index',
            'type'    => 'text',
            'desc'=>__('此为当前专题的期数，比如第二期，请直接填写2','b2'),
        ));
        ...
        if(!$is_circle_default){
            ...
            $cmb_circle->add_field( array( 
                'name'     => __( '如果是免费圈子，请选择入圈规则', 'b2' ), 
                'id'       => 'b2_circle_join_type', 
                'type'     => 'select',
                'options'=>array(
                    'free'=>__('自由入圈','b2'),
                    'check'=>__('用户需要审核入圈','b2')
                ),
                'default'=>''
            ) );
            ...
            $cmb_circle->add_field(array(
                'name' => __('如果是专属圈子，请选择允许入圈的用户组','b2'),
                'id'   => 'b2_circle_join_lv',
                'type' => 'multicheck_inline',
                'desc'=>'<p>'.sprintf(__('如果您修改了此权限，请前往%s圈子设置%s中重置以下该圈子的数据。%s如果没有用户组，请前往%s用户设置%s进行设置','b2'),'<a href="'.admin_url('/admin.php?page=b2_circle_data').'" target="_blank">','</a>','<br>','<a href="'.admin_url().'/admin.php?page=b2_normal_user" target="_blank">','</a>').'</p>',
                'options'=>$setting_lvs,
            ));
    
            $cmb_circle->add_field( array( 
                'name'     => __( '如果是收费圈子，请设置金额（有效期：永久有效）', 'b2' ), 
            ...
            $cmb_circle->add_field( array( 
                'name'     => __( '如果是收费圈子，请设置金额（有效期：按年付费）', 'b2' ), 
            ...
            $cmb_circle->add_field( array( 
                'name'     => __( '如果是收费圈子，请设置金额（有效期：半年付费）', 'b2' ), 
            ...
            $cmb_circle->add_field( array( 
                'name'     => __( '如果是收费圈子，请设置金额（有效期：季度付费）', 'b2' ), 
            ...
            $cmb_circle->add_field( array( 
                'name'     => __( '如果是收费圈子，请设置金额（有效期：按月付费）', 'b2' ), 
            ...
            $cmb_circle->add_field( array( 
                ...
                'type'=>'select',
                'options'=>array(
                    'public'=>__('圈内帖子公开显示','b2'),
                    'private'=>__('圈内帖子只对圈友开放','b2')
                ),
                'after_row'=>'</div></div>',
                'desc'=>sprintf(__('如果您修改了此权限，请前往%s圈子设置%s中重置以下该圈子的数据。%s公开显示：即便用户没有入群，也可以查看圈内帖子，同时也会在广场显示；只对圈友开放：用户入群之后才能查看圈内帖子，不对外开放。','b2'),'<a href="'.admin_url('/admin.php?page=b2_circle_data').'" target="_blank">','</a>','<br>')
            ) );
        }
    ...
    public static function topic_settings(){
        ...
        $topic->add_field( array( 
            ...
            'type' => 'select',
            'desc'=>__('如果使用全局设置，这里请选择全局设置，并且在主题设置->圈子设置->话题设置中编辑全局权限','b2'),
            'options'=>array(
                0=>__('使用全局设置','b2'),
                1=>__('自定义该圈子的发帖权限','b2')
            ),
