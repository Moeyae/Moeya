        ...
        <main id="main" class="site-main b2-radius">
            <div class="box b2-radius" id="carts-list">
                ...
                <div class="carts-pay-resout" v-if="step == 3" v-cloak>
                    ...
                    <p><?php echo sprintf(__('%s请前往订单中心查看已购买订单%s','b2'),'<a :href="orderPage()">','</a>'); ?></p>
                </div>
                <div :class="'step'+step">
                    <ul>
                        <li class="table-bar">
                        ...
                        <li v-else-if="Object.keys(this.data).length == 0" v-cloak>
                            <?php echo B2_EMPTY; ?>
                        </li>
                        <li class="table-info table-bar" v-else v-for="(item,key,index) in data" v-cloak v-if="(pickedProducts(key) && step == 2) || step == 1 || (step == 3 && picked.indexOf(item.id) !== -1)">
                            <label class="carts-action" v-if="step == 1"><input type="checkbox" v-model="picked" :value="item.id" :disabled="id"></label>
                            <div class="carts-name"><img :src="item.thumb"><a :href="item.link" target="_blank"><span v-text="item.title"></a></span></div>
                            <div class="carts-price"><?php echo B2_MONEY_SYMBOL; ?><span v-text="item.price.current_price"></span></div>
                            <div class="carts-count"><button @click="countSub(key)" v-if="step == 1">-</button><span v-text="data[key].count"></span><button @click="countAdd(key)" v-if="step == 1">+</button></div>
                    ...
                    <div class="carts-bottom" v-if="Object.keys(this.data).length != 0 && step != 3" v-cloak>
                        ...
                        <div class="carts-bottom-right">
                            <?php echo __('总价：'); ?><?php echo B2_MONEY_SYMBOL; ?><span v-text="totalMoney()"></span>
                        </div>
                    </div>
                    <div class="carts-comments" v-if="Object.keys(this.data).length != 0 && step != 3" v-cloak>
                        <textarea placeholder="<?php echo __('给卖家留言','b2'); ?>" v-model="comment"></textarea>
                    </div>
                    <input type="password" :style="'position:absolute;top:-999px'" v-cloak/>
                    <div class="carts-address" v-if="showAddress && Object.keys(this.data).length != 0 && step != 3" v-cloak>
                        <div class="">
                            <p class="carts-desc"><?php echo __('实物收货地址（必填）：','b2'); ?></p>
                            <p v-if="pickedAddress !== ''"><span v-text="address.addresses[pickedAddress].address"></span><span v-text="address.addresses[pickedAddress].name"></span><span v-text="address.addresses[pickedAddress].phone"></span></p>
                            <p v-else style="color:#999"><?php echo __('收货地址为空，请添加一个收货地址！','b2'); ?></p>
                        </div>
                        <div class="cart-edit-address"><button @click="showAddressBox = true" class="text"><?php echo b2_get_icon('b2-edit-2-line').__('编辑收货地址','b2'); ?></button></div>
                    </div>
                    <div class="carts-address" v-if="showEmail && Object.keys(this.data).length != 0 && step != 3" v-cloak>
                        <div class="">
                        ...
                        <div class="cart-edit-address"><button @click="showEmailBox = true" class="text"><?php echo b2_get_icon('b2-edit-2-line').__('添加收货邮箱','b2'); ?></button></div>
                    </div>
                    <div class="my-coupons" v-if="coupons !== '' && Object.keys(this.coupons).length > 0 && Object.keys(this.data).length != 0 && step != 3" v-cloak>
                        <ul>
                    ...
                    <div class="carts-address-button" v-if="Object.keys(this.data).length != 0 && step != 3" v-cloak>
