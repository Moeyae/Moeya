            ...
            <?php }else{?>
                <div id="write-head">
                    ...
                    <div class="write-thumb" :style="postData.thumb ? 'min-height:auto' : ''">
                        <label @click="showImgLib" v-if="!postData.thumb && !locked" v-cloak>
                    ...
                    <?php if(!empty($arr_arr)){ ?>
                        <div class="write-select-row-top">
                            ...
                            <div class="cat-picked">
                                <span v-for="item in postData.cats" v-text="catPicked(item)" @click="removeCat(item)" v-if="item"></span>
                            </div>
                    ...
                    <?php } ?>
                    <?php if(!empty($collection_arr)){ ?>
                        <div class="write-select-row-top">
                            <div class="write-select-title"><?php echo __('专题','b2'); ?></div>
                            <div class="cat-picked">
                                <span v-for="item in postData.collections" v-text="collectionPicked(item)" @click="removeCollection(item)" v-if="item"></span>
                            </div>
                            <div class="write-select-box">
                                <select v-model="collection" :disabled="postData.collections.length > 3 ? true : false">
                                    <option disabled :value="0"><?php echo __('专题','b2'); ?></option>
                ...
                <div id="b2-editor-box">
                    ...
                    <div :class="['trix-dialog trix-dialog--images modal',{'show-modal':showImageBox}]" v-cloak>
                        <div class="trix-dialog__link-fields box">
                            ...
                            <span class="close-button" @click="close('image')">×</span>
                            <div class="image-table">
                                <div :class="imgTable == 'upload' ? 'picked' : ''" @click="imgTable = 'upload'"><?php echo __('上传图片','b2'); ?></div>
                                <div :class="imgTable == 'lib' ? 'picked' : ''" @click="imgTable = 'lib'"><?php echo __('我的图片','b2'); ?></div>
                                <div :class="imgTable == 'link' ? 'picked' : ''" @click="imgTable = 'link'" v-if="!thumbPicked"><?php echo __('外链图片','b2'); ?></div>
                            </div>
                            ...
                            <div class="trix-dialog-image-box" v-if="imgTable == 'lib'">
                                <div class="" v-if="imageList.length > 0">    
                                    <ul class="editor-images-list">
                                        <li v-for="item in imageList" @click="item.thumb ? picked('image',item.att_url) : ''" :class="(imagePicked.indexOf(item.att_url) !== -1 && !thumbPicked) || (thumb == item.att_url && thumbPicked) ? 'picked' : ''">
                                    ...
                                    <div class="image-nav" v-if="imagePages > 0">
                                        <button class="text" :disabled="locked.pre || paged == 1 ? true : false" @click="per('image')"><?php echo __('上一页','b2'); ?></button>
                                        <button class="text" :disabled="locked.next || paged == imagePages ? true : false" @click="next('image')"><?php echo __('下一页','b2'); ?></button>
                                    </div>
                                </div>
                            ...
                            <div class="trix-button-group" v-show="imgTable != 'upload'">
                                <button  class="empty" @click="close('image')"><?php echo __('取消','b2'); ?></button>
                                <button @click="insert('image')"  v-if="!thumbPicked"><?php echo __('插入','b2'); ?></button>
                                <button @click="setThumb"  v-else><?php echo __('设置','b2'); ?></button>
                            </div>
                        </div>
                    </div>
                    <div :class="['trix-dialog trix-dialog--video modal',{'show-modal':showVideoBox}]" v-cloak>
                        <div class="trix-dialog__link-fields box">
                            ...
                            <span class="close-button" @click="close('video')">×</span>
                            <div class="image-table">
                                <div :class="videoTable == 'upload' ? 'picked' : ''" @click="videoTable = 'upload'"><?php echo __('上传视频','b2'); ?></div>
                                <div :class="videoTable == 'lib' ? 'picked' : ''" @click="videoTable = 'lib'"><?php echo __('我的视频','b2'); ?></div>
                                <div :class="videoTable == 'link' ? 'picked' : ''" @click="videoTable = 'link'"><?php echo __('外链视频','b2'); ?></div>
                                <div :class="videoTable == 'html' ? 'picked' : ''" @click="videoTable = 'html'"><?php echo __('内嵌视频','b2'); ?></div>
                            </div>
                            ...
                            <div class="trix-dialog-video-box" v-if="videoTable == 'lib'">
                                <div class="" v-if="videoList.length > 0">    
                                    <ul class="editor-images-list">
                                        <li v-for="item in videoList" @click="item.att_url ? picked('video',item.att_url) : ''" :class="videoPicked && videoPicked.indexOf(item.att_url) !== -1 ? 'picked' : ''">
                                            <div class="editor-image">
                                    ...
                                    <div class="image-nav" v-if="videoPages > 0">
                                        <button class="text" :disabled="locked.pre || paged == 1 ? true : false" @click="per('video')"><?php echo __('上一页','b2'); ?></button>
                                        <button class="text" :disabled="locked.next || paged == videoPages ? true : false" @click="next('video')"><?php echo __('下一页','b2'); ?></button>
                                    </div>
                                </div>
                                <div class="editor-file-none" v-else>
                            ...
                            <div class="trix-dialog-input-box" v-if="videoTable == 'html'" v-claok>
                                <div class="trix-dialog-input input-textarea">
                            ...
                            <div class="trix-button-group" v-if="videoTable != 'upload'">
                                <button  class="empty" @click="close('video')"><?php echo __('取消','b2'); ?></button>
                                <button  @click="insert('video')"><?php echo __('插入','b2'); ?></button>
                            </div>
                        </div>
                    </div>
                    <div :class="['trix-dialog trix-dialog--video modal',{'show-modal':showPostBox}]" v-cloak>
                        <div class="trix-dialog__link-fields box">
                            ...
                            <span class="close-button" @click="close('post')">×</span>
                            <div class="trix-dialog-input-box">
                                <div class="trix-dialog-input">
                            ...
                            <div class="trix-button-group">
                                <button  @click="close('post')" class="empty"><?php echo __('取消','b2'); ?></button>
                                <button @click="insertPost()"><?php echo __('插入','b2'); ?></button>
                            </div>
                        </div>
                    </div>
                    <div :class="['trix-dialog trix-dialog--video modal',{'show-modal':showFileBox}]" v-cloak>
                        <div class="trix-dialog__link-fields box">
                            ...
                            <span class="close-button" @click="close('file')">×</span>
                            <div class="trix-dialog-input-box">
                                <div class="trix-dialog-input">
                            ...
                            <div class="trix-button-group">
                                <button @click="close('file')" class="empty"><?php echo __('取消','b2'); ?></button>
                                <button @click="insertFile"><?php echo __('插入','b2'); ?></button>
                            </div>
                        </div>
                    </div>
                </div>
				...
				<input style="position: fixed;left: -10000px" type="password" name="password" id="password_2" />
                    ...
                    ?>
                        <div class="write-select-row" v-for="(item,index) in custom" v-cloak>
                            <div class="write-select-row-title" @click="showSetting(item.key)">
                    ...
                    <div class="write-select-row" v-cloak>
                        <div class="write-select-row-title" @click="showSetting('role')">
                    ...
                    <div class="write-select-row">
                        <div class="write-select-row-title" @click="showSetting('tag')">
                        ...
                        <div class="write-role-settings" v-show="show.tag" v-cloak>
                            ...
                            <label class="write-tags">
                                ...
                                <div class="cat-picked">
                                    <span v-for="(t,index) in tags" @click="removeTag(index)" v-text="t"></span>
                                </div>
                    ...
                    <div class="write-select-row">
                        <div class="write-select-row-title" @click="showSetting('excerpt')">
                    ...
                    <div class="write-bottom">
                        ...
                        <div>
                            <button @click="submit('draft')" :disabled="locked"><?php echo __('保存草稿到服务器','b2'); ?></button>
                            <button @click="submit('publish')" :disabled="locked"><?php echo __('发布','b2'); ?></button>
                        </div>
                    </div>
                </div>
            <?php } ?>
