...
if(!$to){
?>
<div id="primary" class="content-area dmsg-page wrapper" ref="dmsgPage" data-paged="<?php echo $paged; ?>" data-url="<?php echo b2_get_custom_page_url('directmessage'); ?>">
                ...
                <li v-for="(item,index) in list" :id="'dmsg-'+item.id" @click="jump(item.type == 'from' ? item.to.id : item.from.id)">
                    ...
                    <div class="dmsg-row">
                        ...
                        <div class="dmsg-row-bottom">
                            ...
                            <div class="dmsg-tools"><a href="javascript:void(0)" @click="deleteDmsg(item.id)"><?php echo __('删除','b2'); ?></a><span class="unread" v-if="item.status == 0 && item.type !== 'from'" v-cloak><?php echo __('有未读消息','b2'); ?></span></div>
                        </div>
                    </div>
                </li>
            </ul>
...
?>
<div id="primary" class="content-area dmsg-page-to wrapper" ref="mydmsg" data-id="<?php echo $to; ?>" data-paged="<?php echo $paged; ?>">
    <main id="main" class="site-main box b2-radius">
